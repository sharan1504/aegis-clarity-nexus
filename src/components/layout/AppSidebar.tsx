...
export function AppSidebar() {
  ...
  return <Sidebar collapsible="icon">
    <SidebarHeader className="border-b border-sidebar-border"><div className="flex items-center gap-2.5 px-2 py-2.5"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary text-primary-foreground shadow-sm"><Sparkles className="h-4 w-4" /></div>{!collapsed && <div className="flex flex-col leading-tight"><span className="text-sm font-semibold tracking-tight">Cenops</span><span className="text-[10px] uppercase tracking-wider text-muted-foreground">Enterprise AI Ops</span></div>}</div></SidebarHeader>
    <SidebarContent>
      {!collapsed && <SidebarGroup><SidebarGroupContent><GlobalSearch /></SidebarGroupContent></SidebarGroup>}
      {!collapsed && <SidebarGroup><SidebarGroupContent><Link to="/chat" className="flex w-full items-center gap-2 rounded-md bg-primary px-3 py-2 text-sm font-medium text-primary-foreground shadow-sm hover:bg-primary/90"><Sparkles className="h-4 w-4" /><span>Ask Cenops</span></Link></SidebarGroupContent></SidebarGroup>}
      {nav.map((group) => <SidebarGroup key={group.section}>{!collapsed && <SidebarGroupLabel>{group.section}</SidebarGroupLabel>}<SidebarGroupContent><SidebarMenu>{group.items.map((item) => <SidebarMenuItem key={item.url}><SidebarMenuButton asChild isActive={isActive(item.url)} tooltip={item.title}><Link to={item.url} className="flex items-center gap-2"><item.icon className="h-4 w-4 shrink-0" />{!collapsed && <span className="flex min-w-0 flex-1 items-center gap-2">{item.title}{item.url === "/operational-console" && openHighCritical > 0 && <Badge variant="destructive" className="ml-auto h-5 min-w-5 px-1 text-[10px]">{openHighCritical}</Badge>}</span>}</Link></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu></SidebarGroupContent></SidebarGroup>)}
    </SidebarContent>
