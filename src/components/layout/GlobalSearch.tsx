...
import { Bot, BarChart3, Gauge, History, Plug, Search, SearchCheck, Settings, ShieldCheck, Sparkles, Users, LayoutDashboard } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { useTenantContext } from "@/lib/tenant";

type SearchItem = { label: string; description: string; href: string; group: string; icon: typeof Search };
...
export function GlobalSearch() {
  ...
  useEffect(() => {
    ...
    if (!tenantId) return;
    void Promise.all([
      supabase.from("integrations").select("display_name,provider,environment,status").eq("tenant_id", tenantId).order("display_name").limit(40),
      ...
      if (!active) return;
      const connected: SearchItem[] = (integrations.data ?? []).map((x) => ({ label: x.display_name ?? x.provider, description: `${x.provider} · ${x.environment ?? "Production"} · ${x.status}`, href: "/integrations", group: "Connected systems", icon: Plug }));
  ...
  return <>
    <button type="button" onClick={() => setOpen(true)} className="group flex w-full items-center gap-2 rounded-md px-2 py-2 text-sm text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground"><Search className="h-4 w-4" /><span className="flex-1 text-left">Search</span><kbd className="rounded border bg-background/60 px-1.5 py-0.5 text-[10px] text-muted-foreground">Ctrl K</kbd></button>
    <CommandDialog open={open} onOpenChange={setOpen}><CommandInput placeholder="Search Aegis, features, agents, integrations, changes…" /><CommandList><CommandEmpty>No matching Aegis resources found.</CommandEmpty>{Object.entries(groups).map(([group, groupItems]) => <CommandGroup key={group} heading={group}>{groupItems.map((item) => { const Icon = item.icon; return <CommandItem key={`${group}:${item.label}`} value={`${item.label} ${item.description}`} onSelect={() => { setOpen(false); void navigate({ to: item.href as never }); }}><Icon /><span className="min-w-0"><span className="block truncate">{item.label}</span><span className="block truncate text-xs text-muted-foreground">{item.description}</span></span></CommandItem>; })}</CommandGroup>)}</CommandList></CommandDialog>
  </>;
}
