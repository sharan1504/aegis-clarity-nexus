...
export function brokeredPreviewStorage() {
  ...
  const projectId = onPreviewZone ? (host.match(new RegExp('^(?:id-preview(?:-[a-z0-9]+)?|project)--(' + UUID + ')(?:-dev)?(?=\\.|$)', 'i'))?.[1] ?? host.match(new RegExp('^(' + UUID + ')(?=[.-])', 'i'))?.[1]) : undefined;
  ...
  const request = (type: string, key: string, value?: string): Promise<{ ok: boolean; value?: string | null } | null> => new Promise((resolve) => {
    ...
    const finish = (r: { ok: boolean; value?: string | null } | null) => { if (done) return; done = true; window.removeEventListener('message', onMessage); resolve(r); };
  ...
  return {
    getItem: async (key: string) => { let res = await request('lovable-preview-auth:get', key); if (!res && firstGet) { await new Promise((r) => setTimeout(r, RETRY_DELAY)); res = await request('lovable-preview-auth:get', key); } firstGet = false; if (res && res.ok && typeof res.value === 'string') { if (res.value === '') { localStorage.removeItem(key); return null; } return res.value; } return localStorage.getItem(key); },
