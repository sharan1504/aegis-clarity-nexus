...
function demoAgentDetail(agentKey: string): AgentDetail | null {
  ...
  return {
    ...
    operational: true,
    telemetry: { aiRequests: usage.length ? usage.length * 18 : 24, totalTokens: usage.reduce((sum, row) => sum + row.total_tokens, 0) || 18420, averageLatencyMs: usage.length ? Math.round(usage.reduce((sum, row) => sum + row.latency_ms, 0) / usage.length) : 940, firstActivityAt: "2026-09-03T06:00:00.000Z", lastActivityAt: environmentMode === "demo" ? DEMO_NOW : null, telemetryAvailable: true },
