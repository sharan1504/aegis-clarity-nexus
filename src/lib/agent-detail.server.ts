...
function demoAgentDetail(agentKey: string): AgentDetail | null {
  const meta = DEMO_AGENT_META[agentKey]; const workflow = DEMO_AGENT_WORKFLOWS[agentKey] ?? { trigger: "Manual run", description: `Evaluate the ${meta?.name ?? "demo agent"} using deterministic workspace evidence.`, config: { approvalMode: "required" }, steps: [{ id: `${agentKey}-1`, name: "Load demo evidence", type: "evidence", provider: "CenOps Demo", capability: "demo_evidence", action: "Read deterministic demo evidence for this agent." }, { id: `${agentKey}-2`, name: "Prepare governed result", type: "decision", action: "Prepare an approval-gated recommendation without provider mutation.", requiresApproval: true }, { id: `${agentKey}-3`, name: "Verify result", type: "verification", action: "Record the deterministic demo result.", verification: "Demo result remains read-only and provider-safe." }] }; if (!meta) return null;
  ...
  return { agentKey, displayName: meta.name, description: meta.description, category: meta.category, bindings: [{ integrationId: `demo-${agentKey}`, provider: workflow.steps.find((s) => s.provider)?.provider ?? "CenOps Demo", capabilityKey: workflow.steps.find((s) => s.capability)?.capability ?? "workflow", capabilityName: "Demo capability", enabled: true, isMock: true }], operational: true, telemetry: { aiRequests: usage.length ? usage.length * 18 : 24, totalTokens: usage.reduce((sum, row) => sum + row.total_tokens, 0) || 18420, averageLatencyMs: usage.length ? Math.round(usage.reduce((sum, row) => sum + row.latency_ms, 0) / usage.length) : 940, firstActivityAt: "2026-09-03T06:00:00.000Z", lastActivityAt: DEMO_NOW, telemetryAvailable: true }, changes: changes.map((row) => ({ rowId: row.id, changeId: row.changeId, title: row.title, stage: row.stage, severity: row.severity, createdAt: row.createdAt, savings: row.id === "demo-change-1" ? "USD 6,120" : row.id === "demo-change-2" ? "USD 3,480" : NOT_ESTIMATED })), activity: activity.map((row) => ({ action: row.action, detail: row.detail, actor: row.actor, createdAt: row.createdAt })), savings: { summary: agentKey === "agent-license" ? "USD 6,120" : agentKey === "agent-cost" ? "USD 3,480" : NOT_ESTIMATED, entries: agentKey === "agent-license" ? [{ currency: "USD", amount: 6120 }] : agentKey === "agent-cost" ? [{ currency: "USD", amount: 3480 }] : [] }, instructions: { pre: "Verify evidence freshness before analysis.", system: `Operate as the ${meta.name}. Use only authorized connected capabilities.`, post: "Return evidence, confidence, risk, action and verification status." }, workflow: workflow as unknown as AgentWorkflow, generatedAt: DEMO_NOW };
}
