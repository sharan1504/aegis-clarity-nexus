...
async function saveEvidence(supabase: any, tenantId: string, runId: string, evidence: JsonValue[]) {
  for (const item of evidence) {
    const { error } = await supabase.from("agent_run_evidence").insert({ run_id: runId, tenant_id: tenantId, evidence: item });
    if (error) throw new Error(error.message);
  }
}
