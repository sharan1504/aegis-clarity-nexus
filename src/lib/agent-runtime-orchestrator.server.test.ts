...
describe("security runtime orchestrator", () => {
  it("collects real routed evidence, evaluates policy, then waits for approval", async () => {
    ...
    vi.mocked(githubCapabilityRouter.getSecurityFindings).mockResolvedValue({
      tenantId: "tenant-1", agentKey: "agent-security", records: [{ integrationId: "github-1", findingId: "f-1" }], sources: [], warnings: [], evaluatedAt: clock.now(), freshness: "fresh",
      policies: { "github-1": { policy: { approval_required: true }, revision: { version: 1 } } },
    } as never);
    vi.mocked(analyzeSecurityFindings).mockReturnValue({
      recommendations: [{ findingId: "f-1" }], excludedCount: 0, evaluatedCount: 1, exceededRepositoryCeiling: false,
    } as never);
