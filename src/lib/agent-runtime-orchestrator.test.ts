...
describe("agent runtime orchestrator", () => {
  it("runs evidence and policy deterministically", () => {
    const result = orchestrateAgentRun(createRun(), [{ type: "investigate", value: { findingId: "f-1" } }, { type: "policy", value: { verdict: "allow" } }], clock);
  ...
  it("halts at approval and cannot execute past the gate", () => {
    const result = orchestrateAgentRun(createRun(), [{ type: "investigate", value: { findingId: "f-1" } }, { type: "policy", value: { verdict: "allow" } }, { type: "await_approval", value: { status: "pending" } }, { type: "execute", value: { status: "should-not-run" } }], clock);
  ...
  it("requires explicit approval before execution and verification", () => {
    const result = orchestrateAgentRun(createRun(), [{ type: "investigate", value: { findingId: "f-1" } }, { type: "policy", value: { verdict: "allow" } }, { type: "await_approval", value: { status: "pending" } }, { type: "approve", value: { status: "approved", approvedBy: "user-1" } }, { type: "execute", value: { status: "executed" } }, { type: "verify", value: { status: "verified" } }], clock);
