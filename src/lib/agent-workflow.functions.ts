...
export const saveAgentWorkflow = createServerFn({ method: "POST" })
  ...
  .inputValidator((input: { agentKey: string; trigger: string; config: Record<string, unknown>; steps: AgentWorkflowStepInput[] }) => ({
    agentKey: String(input?.agentKey ?? "").trim(), trigger: String(input?.trigger ?? "").trim(), config: input?.config ?? {}, steps: Array.isArray(input?.steps) ? input.steps : [],
  }))
  .handler(async ({ data, context }) => {
    ...
    const { error: configError } = await db.from("agent_workflow_configs").upsert({ tenant_id: tenantId, agent_key: data.agentKey, trigger_config: { trigger: data.trigger }, policy: data.config, updated_by: context.userId }, { onConflict: "tenant_id,agent_key" });
