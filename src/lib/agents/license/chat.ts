...
const LOVABLE_AI_MODEL = "google/gemini-3-flash";
...
function parseIntent(raw: string): Intent {
  ...
  return {
    operation: parsed.operation as Intent["operation"],
...
async function collectEvidence(intent: Intent, supabase: Parameters<typeof capabilityRouter.getUsers>[0], userId: string) {
  ...
  if (intent.operation === "summary") {
    const [licenses, users] = await Promise.all([
      capabilityRouter.getLicenseInventory(supabase, userId, LICENSE_AGENT_KEY, { now }),
      capabilityRouter.getUsers(supabase, userId, LICENSE_AGENT_KEY, { now }),
    ]);
  ...
  if (intent.operation === "usage") {
    const result = await executeLicenseAgent({ data: { operation: "get_license_usage", filters: { licenseId: intent.licenseId, licenseName: intent.licenseName } } } as never);
    return { type: "usage", result };
  }

  if (intent.operation === "assignments") {
    const result = await executeLicenseAgent({ data: { operation: "get_license_assignments", filters: { licenseId: intent.licenseId, licenseName: intent.licenseName, userId: intent.userId, userEmail: intent.userEmail } } } as never);
    return { type: "assignments", result };
  }

  if (intent.operation === "user_details") {
    const result = await executeLicenseAgent({ data: { operation: "get_user_license_details", filters: { userId: intent.userId, userEmail: intent.userEmail } } } as never);
    return { type: "user_details", result };
  }

  const optimization = await executeLicenseOptimization({} as never);
  const summary = await executeLicenseAgent({ data: { operation: "get_license_summary" } } as never);
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      const response = await askModel([
        {
          role: "system",
          content: "You are the License Agent. Answer only from the supplied structured evidence. Do not invent facts, dates, usage, savings, license changes, or customer configuration. If the evidence cannot answer the question, say: 'There is not sufficient information available in the connected data to make a reliable recommendation for this request.' Distinguish a review opportunity from a recommendation to change a license. Never claim to have performed a license change. Be concise and business-friendly. Mention relevant evidence and limitations.",
        },
        {
          role: "user",
          content: JSON.stringify({ question: latest, conversation: data.messages.slice(-8), evidence }),
        },
      ]);
