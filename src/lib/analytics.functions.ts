...
export const getAnalytics = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [audit, users, roles, agents, changes, aiUsage, settings] = await Promise.all([
      ...
      db.from("change_records").select("agent,stage,severity,created_at").eq("tenant_id", tenantId).gte("created_at", since),
    ...
    for (const row of changeRows) {
      ...
      const current = agentMap.get(key) ?? { name: key, category: "Operations", actions: 0, changes: 0, tokens: 0 };
      current.changes += 1; agentMap.set(key, current);
    }
    for (const row of usageRows) {
      const current = agentMap.get(row.agent_key) ?? { name: row.agent_key, category: "AI", actions: 0, changes: 0, tokens: 0 };
      current.actions += 1; current.tokens += Number(row.total_tokens ?? 0); agentMap.set(row.agent_key, current);
    }
    ...
    return {
      ...
      platform: {
        ...
        connectedRoles: new Set(roleRows.map((x: any) => x.role)).size,
      ...
      userActivity: {
        additions: byAction(/user\.(added|created|invited)/i),
        removals: byAction(/user\.(removed|deleted)/i),
        updates: byAction(/user\.(updated|role_changed|changed)/i),
        uniqueActors: new Set(auditRows.map((x: any) => x.actor_email).filter(Boolean)).size,
      },
      ...
      governance: {
        serviceLevel: analyticsSettings.serviceLevel ?? { targetSeconds: 30, targetPercent: 80 },
        dataMasking: analyticsSettings.dataMasking ?? true,
        disconnectMetricWindowMinutes: analyticsSettings.disconnectMetricWindowMinutes ?? 30,
        retentionDays: analyticsSettings.retentionDays ?? 90,
      },
...
export const updateAnalyticsSettings = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!roles.includes("admin")) throw new Error("Only workspace administrators can update analytics settings.");
