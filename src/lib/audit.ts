...
export async function fetchAuditTrail(tenantId: string, opts: { entityId?: string; limit?: number } = {}): Promise<AuditEntry[]> {
  let query = supabase
    .from("audit_log")
    .select("id, action, entity_type, entity_id, actor_email, actor_role, detail, hash, prev_hash, created_at")
  ...
  if (error) throw error;
  return (data ?? []).map((row) => ({ id: row.id, action: row.action, entityType: row.entity_type, entityId: row.entity_id, actorEmail: row.actor_email, actorRole: row.actor_role, detail: row.detail, hash: row.hash, prevHash: row.prev_hash, createdAt: row.created_at }));
}
