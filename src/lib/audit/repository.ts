...
async function currentUserId() { const { data: auth } = await supabase.auth.getUser(); return auth.user?.id ?? null; }
async function currentTenantId() { const userId = await currentUserId(); if (!userId) return null; const { tenantId } = await resolveTenantContext(supabase, userId); return tenantId; }
...
/**
 * The implementation is selected for every operation from the authenticated
 * tenant's persisted environment mode. It is deliberately not a module-level
 * ternary, so one tenant can never leak demo data into another tenant's request.
 */
...
async function resolveAuditRepository(): Promise<AuditRepository> {
  ...
  if (!userId) return new SupabaseAuditRepository();
