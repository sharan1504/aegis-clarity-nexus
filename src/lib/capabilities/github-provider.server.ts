...
async function providerConnectionId(db: any, source: AuthorizedSource, tenantId: string): Promise<string> {
  const { data, error } = await db.from("provider_connections").select("id").eq("tenant_id", tenantId).eq("integration_id", source.integrationId).eq("provider", "github").maybeSingle();
  if (error) throw error;
  if (!data?.id) throw new Error("The GitHub provider connection is not linked to this agent integration source.");
...
export async function getGitHubRepositories(db: any, source: AuthorizedSource, tenantId: string): Promise<NormalizedRepository[]> {
  const connectionId = await providerConnectionId(db, source, tenantId);
  const { data, error } = await db.from("github_synced_entities").select("entity_key, repository_name, html_url, provider_updated_at, synced_at, snapshot_id, sync_id, stale, payload").eq("tenant_id", tenantId).eq("connection_id", connectionId).eq("entity_type", "repository").eq("stale", false);
  if (error) throw error;
...
export async function getGitHubSecurityFindings(db: any, source: AuthorizedSource, tenantId: string): Promise<NormalizedSecurityFinding[]> {
  const connectionId = await providerConnectionId(db, source, tenantId);
  const { data, error } = await db.from("github_synced_entities").select("entity_key, repository_name, alert_severity, alert_title, alert_state, html_url, provider_updated_at, synced_at, snapshot_id, sync_id, stale, payload").eq("tenant_id", tenantId).eq("connection_id", connectionId).eq("entity_type", "security_alert").eq("stale", false);
  if (error) throw error;
  return (data ?? []).map((row: Record<string, any>) => { const p = row.payload && typeof row.payload === "object" ? row.payload : {}; const kind = p.kind === "code_scanning" ? "code_scanning" : "dependabot"; const raw = String(row.alert_severity ?? p.severity ?? "").toLowerCase(); const severity = (["low", "medium", "high", "critical"].includes(raw) ? raw : "unknown") as NormalizedFindingSeverity; return { provider: "github", integrationId: source.integrationId, findingId: String(row.entity_key), repositoryName: row.repository_name ?? null, findingType: kind, title: row.alert_title ?? (typeof p.title === "string" ? p.title : null), severity, state: row.alert_state ?? (typeof p.state === "string" ? p.state : null), url: row.html_url ?? (typeof p.htmlUrl === "string" ? p.htmlUrl : null), providerUpdatedAt: row.provider_updated_at ?? null, metadata: { dependency: p.dependency ?? null, advisory: p.advisory ?? null }, provenance: provenanceFor(source, row) } satisfies NormalizedSecurityFinding; });
}
