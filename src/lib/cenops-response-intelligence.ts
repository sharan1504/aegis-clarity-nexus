...
export interface CenOpsKeyFinding {
  title: string;
  detail: string;
  severity?: CenOpsSeverity;
  status?: string;
}

export interface CenOpsMetric {
  label: string;
  value: string;
  change?: string;
  trend?: "up" | "down" | "flat";
...
export interface CenOpsOpportunity {
  title: string;
  value?: string;
  rationale: string;
  evidence: string[];
...
export interface CenOpsRecommendation {
  title: string;
  rationale: string;
  impact: string;
  risk: string;
...
export interface CenOpsEvidence {
  source: string;
  detail: string;
  timestamp?: string;
}

export interface CenOpsFollowUp {
  label: string;
  prompt: string;
  route?: string;
}
...
export function normalizeCenOpsResponse(raw: unknown, intent: string): CenOpsResponse {
  ...
  const recommendations = asArray(value.recommendations, (item) => {
    const r = (item && typeof item === "object" ? item : {}) as Record<string, unknown>;
    return { title: asString(r.title, "Recommended next step"), rationale: asString(r.rationale), impact: asString(r.impact, "Operational improvement"), risk: asString(r.risk, "Not assessed"), nextStep: asString(r.nextStep, asString(r.rationale)), actionType: asString(r.actionType) || undefined, requiresApproval: r.requiresApproval === true };
  });
  return {
    responseType: type,
