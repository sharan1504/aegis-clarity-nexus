...
export interface ProposedChangeInput { title: string; businessImpact: string; aiReasoning: string; proposedRiskFactors: string[]; targetProvider: string; targetAgent: string; targetIntegrationId?: string | null; agentRunId?: string | null; proposedRiskTier?: "Low" | "Medium" | "High" | "Critical"; }
...
export async function createProposedChangeRecord(supabase: UserClient, actor: { userId: string; tenantId: string; actorRole: string }, input: ProposedChangeInput) {
  const title = input.title.trim(); const businessImpact = input.businessImpact.trim(); const aiReasoning = input.aiReasoning.trim(); const provider = input.targetProvider.trim(); const agent = input.targetAgent.trim(); const agentRunId = input.agentRunId?.trim() || null; const integrationId = input.targetIntegrationId?.trim() || null; const factors = [...new Set(input.proposedRiskFactors.map((factor) => factor.trim()).filter(Boolean))];
  ...
  const recordQuery = (supabase as any).from("change_records").insert(insert).select("id,change_id,stage").single(); const { data: record, error } = await recordQuery; if (error || !record) throw new Error(error?.message ?? "Could not create the proposed change record.");
  ...
  await writeAuditServer(supabase, { tenantId: actor.tenantId, action: "change.proposed", entityType: "change_record", entityId: record.change_id, detail: `${title} — proposed`, payload: { provider, agent, integrationId, agentRunId, riskFactors: factors, stage: initialStage, actorRole: actor.actorRole, autoGenerateRollbackPlans } });
