...
export interface ProposedChangeInput { title: string; businessImpact: string; aiReasoning: string; proposedRiskFactors: string[]; targetProvider: string; targetAgent: string; agentRunId?: string | null; proposedRiskTier?: "Low" | "Medium" | "High" | "Critical"; }
...
export async function createProposedChangeRecord(supabase: UserClient, actor: { userId: string; tenantId: string; actorRole: string }, input: ProposedChangeInput) {
  ...
  await writeAuditServer(supabase, { tenantId: actor.tenantId, action: "change.proposed", entityType: "change_record", entityId: record.change_id, detail: `${title} — proposed`, payload: { provider, agent, agentRunId, riskFactors: factors, stage: initialStage, actorRole: actor.actorRole, autoGenerateRollbackPlans } });
