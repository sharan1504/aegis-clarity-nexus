...
export async function decideChange(record: ChangeRecord, decision: "approved" | "rejected", ctx: ActorContext, comment?: string) {
  ...
  if (DEMO_DATA_ENABLED) {
    ...
    updateRecords((records) => records.map((current) => current.id !== record.id ? current : { ...current, stage: stage ?? (decision === "rejected" ? "Closed" : current.stage), approvals: current.approvals.map((approval) => approval.status === "pending" ? { ...approval, status: decision, timestamp: now, comment: comment ?? `${decision === "approved" ? "Approved" : "Rejected"} by ${ctx.actor} (${ctx.role})` } : approval), timeline: [{ ts: now, actor: ctx.actor, kind: "status", text: decision === "approved" ? `Approval recorded by ${ctx.actor} (${ctx.role}).${stage ? ` Stage advanced to ${stage}.` : ""}` : `Change rejected by ${ctx.actor} (${ctx.role}).` }, ...current.timeline] }));
