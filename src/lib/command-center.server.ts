...
export interface CommandCenterChange {
  id: string;
  changeId: string;
  title: string;
  stage: string;
...
export interface CommandCenterData {
  /** Live provider telemetry — the "what is happening" layer. */
  live: LiveWorkspaceData;
  attention: {
    pendingChanges: number;
    proposedChanges: number;
    blockingGuardrailEvaluations: number;
  ...
  risk: {
    ...
    guardrailsMonitoringOnly: number;
  };
  posture: {
    integrations: Array<{ id: string; provider: string; status: string; healthStatus: string; lastSyncAt: string | null; lastSyncStatus: string | null; isMock: boolean }>;
    agentsWithRealBindings: number;
...
/** Aggregates only rows that already exist in the tenant's own tables. No synthetic values are added. */
export async function loadCommandCenterData(supabase: UserClientLike, userId: string): Promise<CommandCenterData> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Supabase generated types do not cover every runtime-selected column set here.
  ...
  return {
    ...
    changed: changeRows.slice(0, 8).map((row) => ({
      id: String(row.id),
      changeId: String(row.change_id),
      title: String(row.title),
      stage: String(row.stage),
      severity: String(row.severity ?? "unspecified"),
      ownerTeam: String(row.owner_team ?? "Unassigned"),
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- rows are shaped by the selects above.
    signals: (auditRows.data ?? []).map((row: any) => ({
      id: String(row.id),
      action: String(row.action),
      entityType: String(row.entity_type),
      entityId: row.entity_id ? String(row.entity_id) : null,
      detail: row.detail ? String(row.detail) : null,
      actor: row.actor_email ? String(row.actor_email) : null,
      createdAt: String(row.created_at),
    })),
