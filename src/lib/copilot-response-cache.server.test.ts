...
import {
  clearCopilotResponseCache,
  copilotCacheKey,
  COPILOT_CACHE_TTL_MS,
  getCachedCopilotResponse,
  isStableCopilotCacheCandidate,
  setCachedCopilotResponse,
} from "./copilot-response-cache.server";

describe("copilot response cache", () => {
  beforeEach(() => clearCopilotResponseCache());

  it("only allows stable product questions without volatile tenant scope", () => {
  ...
  it("isolates entries by tenant, environment, intent and request", () => {
  ...
  it("expires entries after the ten-minute TTL", () => {
    vi.useFakeTimers();
    try {
