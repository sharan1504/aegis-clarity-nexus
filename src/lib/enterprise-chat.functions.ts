...
import { normalizeCenOpsResponse, type CenOpsResponse } from "@/lib/cenops-response-intelligence";
...
async function askModel(messages: Array<{ role: "system" | "user" | "assistant"; content: string }>) {
  ...
  const response = await fetch(LOVABLE_AI_ENDPOINT, { method: "POST", headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" }, body: JSON.stringify({ model: LOVABLE_AI_MODEL, messages, temperature: 0.1, response_format: { type: "json_object" } }) });
  const text = await response.text();
  if (!response.ok) { console.error("[enterprise-chat][ai-gateway] request rejected", { status: response.status, model: LOVABLE_AI_MODEL, endpoint: LOVABLE_AI_ENDPOINT, body: text.slice(0, 1000) }); throw new Error(describeAiGatewayError(response.status, text, LOVABLE_AI_MODEL)); }
...
export const executeEnterpriseChat = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input: { sessionId: string; messages: EnterpriseChatMessage[] }) => ({ sessionId: String(input?.sessionId ?? "").trim(), messages: Array.isArray(input?.messages) ? input.messages : [] })).handler(async ({ data, context }) => {
  ...
  try {
    ...
    const [live, providers] = shouldLoadLiveEvidence ? await Promise.all([runEvidenceTool("Genesys", "cenops-workspace", "loadLiveWorkspaceData", { departmentKey: department.departmentKey }, () => loadLiveWorkspaceData(context.supabase, context.userId, department.departmentKey)), runEvidenceTool("Connected providers", "cenops-provider-evidence", "loadProviderReportData", { departmentKey: department.departmentKey }, () => loadProviderReportData(context.supabase, context.userId, department.departmentKey))]) : [{}, { entities: [], providers: [], fetchedAt: new Date().toISOString() }];
    ...
    const safeResult = { ...raw, demo: false, answer: response.executiveSummary, recommendations: response.recommendations, sources: response.evidence.map((e) => e.source), confidence: response.confidence, actionRequired: response.actionRequired, intent: intent.intent, intentConfidence: intent.confidence, correlatedSignals: safeCorrelations, response, department: department.departmentName ?? "Workspace-wide" };
    if (investigationId) await recordInvestigationStep(db, investigationId, tenantId, { stepNumber: stepNumber++, stepType: "finding", name: "AI investigation and structured findings", provider: "Lovable AI", toolName: "enterprise_model", input: { model: LOVABLE_AI_MODEL, intent: intent.intent }, output: safeResult, finding: response.executiveSummary });
    const { error: assistantMessageError } = await db.from("chat_messages").insert({ tenant_id: tenantId, session_id: data.sessionId, user_id: context.userId, role: "assistant", content: response.executiveSummary, result: safeResult });
    if (assistantMessageError) throw new Error(assistantMessageError.message);
    if (investigationId) { await recordInvestigationStep(db, investigationId, tenantId, { stepNumber: stepNumber++, stepType: "response", name: "Customer response generated", output: { answer: response.executiveSummary, confidence: response.confidence, sources: safeResult.sources } }); await completeCustomerInvestigation(db, investigationId, investigationTenantId!, { status: response.actionRequired ? "needs_human" : "resolved", intent: intent.intent, resolution: response.executiveSummary, confidence: response.confidence, channel: "chat", responseText: response.executiveSummary, evidenceSummary: { sources: safeResult.sources, correlations: safeCorrelations }, verification: false }); }
    return { ok: true as const, ...safeResult, provider: "Lovable AI", model: LOVABLE_AI_MODEL, readOnly: true as const, fetchedAt: shouldLoadLiveEvidence ? live.fetchedAt : new Date().toISOString(), investigationId };
  } catch (error) {
