...
import { completeCustomerInvestigation, recordInvestigationStep, recordToolInvocation, startCustomerInvestigation } from "@/lib/customer-investigation.server";
...
export const executeEnterpriseChat = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      } catch (error) { console.error("[customer-investigation] could not initialize", error); }

      const liveStarted = Date.now();
      const providerStarted = Date.now();
      const [live, providers] = await Promise.all([loadLiveWorkspaceData(context.supabase, context.userId, department.departmentKey), loadProviderReportData(context.supabase, context.userId, department.departmentKey)]);
      if (investigationId) {
        try {
          await Promise.all([
            recordToolInvocation(db, { tenantId, investigationId, conversationId: data.sessionId, interactionId: data.sessionId, userId: context.userId }, { provider: "Genesys", serverName: "aegis-workspace", toolName: "loadLiveWorkspaceData", arguments: { departmentKey: department.departmentKey }, result: live, status: "success", startedAt: liveStarted }),
            recordToolInvocation(db, { tenantId, investigationId, conversationId: data.sessionId, interactionId: data.sessionId, userId: context.userId }, { provider: "Connected providers", serverName: "aegis-provider-evidence", toolName: "loadProviderReportData", arguments: { departmentKey: department.departmentKey }, result: providers, status: "success", startedAt: providerStarted }),
          ]);
          await recordInvestigationStep(db, investigationId, tenantId, { stepNumber: stepNumber++, stepType: "evidence", name: "Authorized enterprise evidence gathered", provider: "Aegis evidence layer", output: { genesys: live, providers }, finding: "Evidence was gathered within the caller's department scope." });
        } catch (error) { console.error("[customer-investigation] evidence recording failed", error); }
      }
      ...
      if (investigationId) {
        try {
          await recordToolInvocation(db, { tenantId, investigationId, conversationId: data.sessionId, interactionId: data.sessionId, userId: context.userId }, { provider: "Lovable AI", serverName: "ai.gateway.lovable.dev", toolName: "enterprise_model", arguments: { model: MODEL, temperature: 0.1, responseFormat: "json_object" }, result: parsed, status: "success", startedAt: Date.now() - result.latencyMs });
        ...
        } catch (error) { console.error("[customer-investigation] model evidence recording failed", error); }
      }
      ...
      if (investigationId) {
        ...
        } catch (error) { console.error("[customer-investigation] completion recording failed", error); }
      }
