...
export interface IntegrationSummary {
  id: string | null; provider: string; status: string; healthStatus: string; healthDetail: string | null; region: string | null;
  externalOrgId: string | null; externalOrgName: string | null; scopes: string[]; lastSyncAt: string | null;
  lastSyncStatus: string | null; lastSyncError: string | null; connectedAt: string | null;
...
export async function consumeOAuthState(state: string, tenantId: string): Promise<{ region: string; redirectUri: string }> {
  const db = await admin();
  const { data } = await db.from("integration_oauth_states").select("state, tenant_id, region, redirect_uri, expires_at, consumed_at").eq("state", state).maybeSingle();
...
async function connectionRow(integrationId: string, tenantId: string) {
  const db = await admin();
  const { data, error } = await (db.from("provider_connections" as any) as any).select("id,encrypted_credentials").eq("integration_id", integrationId).eq("tenant_id", tenantId).eq("provider", PROVIDER).maybeSingle();
  if (error) throw new IntegrationError("provider_error", error.message);
  if (!data) throw new IntegrationError("not_connected");
  return data as { id: string; encrypted_credentials: string | null };
}
...
export async function saveClientCredentials(integrationId: string, tenantId: string, credentials: { clientId: string; clientSecret: string }) {
  const db = await admin();
  const encrypted = encryptCredentials({ provider: PROVIDER, ...credentials });
  const row = await connectionRow(integrationId, tenantId).catch(() => null);
  const payload: Record<string, unknown> = { tenant_id: tenantId, provider: PROVIDER, integration_id: integrationId, display_name: "Genesys Cloud", status: "failed", encrypted_credentials: encrypted, updated_at: new Date().toISOString() };
  const query = db.from("provider_connections" as any) as any;
  const result = row ? await query.update(payload).eq("id", row.id).eq("tenant_id", tenantId) : await query.insert(payload);
...
export async function getStoredCredentials(integrationId: string, tenantId: string) {
  const row = await connectionRow(integrationId, tenantId);
  if (!row.encrypted_credentials) throw new IntegrationError("not_configured", "The encrypted OAuth client configuration is missing. Reconnect the integration.");
...
export async function saveTokens(integrationId: string, tenantId: string, tokens: genesys.GenesysTokens) {
  const db = await admin();
  const current = await getStoredCredentials(integrationId, tenantId);
  const encrypted = encryptCredentials({ ...current, accessToken: tokens.accessToken, refreshToken: tokens.refreshToken, tokenType: tokens.tokenType, expiresAt: tokens.expiresAt, scopes: tokens.scopes });
  const row = await connectionRow(integrationId, tenantId);
...
/** Returns a usable access token, refreshing it first when it is within two minutes of expiry. */
export async function getAccessToken(integrationId: string, tenantId: string, region: string | null): Promise<string> {
  const credentials = await getStoredCredentials(integrationId, tenantId);
  if (!credentials.accessToken) throw new IntegrationError("not_connected");
  ...
  if (!credentials.refreshToken) {
    await markIntegration(integrationId, { status: "action_required", health_status: "unhealthy", health_detail: "Genesys access token expired and no refresh token is available. Reconnect required." });
    throw new IntegrationError("token_expired");
  }
  try {
    const refreshed = await genesys.refreshAccessToken({ refreshToken: credentials.refreshToken, regionId: region, });
    await saveTokens(integrationId, tenantId, { ...refreshed, refreshToken: refreshed.refreshToken ?? credentials.refreshToken });
    return refreshed.accessToken;
  } catch (error) {
    await markIntegration(integrationId, { status: "action_required", health_status: "unhealthy", health_detail: "Genesys token refresh failed. Reconnect required." });
    throw error;
  }
}

export async function markIntegration(integrationId: string, patch: Record<string, unknown>) {
  const db = await admin(); await db.from("integrations").update(patch as never).eq("id", integrationId);
}
...
export async function runSync(supabase: UserClient, userId: string, tenantId: string, integrationId: string, region: string | null): Promise<SyncResult> {
  const db = await admin(); const startedAt = new Date().toISOString();
  const { data: run } = await db.from("integration_sync_runs").insert({ tenant_id: tenantId, integration_id: integrationId, trigger: "manual", status: "running", started_at: startedAt, created_by: userId }).select("id").single();
  ...
  try {
    ...
    { const { error } = await db.from("genesys_user_licenses").delete().eq("integration_id", integrationId).lt("synced_at", syncedAt); if (error) throw new IntegrationError("provider_error", error.message); }
    ...
    if (run?.id) await db.from("integration_sync_runs").update({ status: "success", finished_at: finishedAt, stats: counts as never }).eq("id", run.id);
    ...
    const code = toErrorCode(error); const message = toErrorMessage(error); const finishedAt = new Date().toISOString();
    if (run?.id) await db.from("integration_sync_runs").update({ status: "failed", finished_at: finishedAt, error_code: code, error_message: message, stats: counts as never }).eq("id", run.id);
...
export async function disconnect(supabase: UserClient, tenantId: string, integrationId: string) {
  const db = await admin();
  const row = await connectionRow(integrationId, tenantId).catch(() => null);
  if (row) await (db.from("provider_connections" as any) as any).delete().eq("id", row.id).eq("tenant_id", tenantId);
