...
// All credential-bearing logic stays server-side and uses provider_connections.
...
export const startGenesysOAuth = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input: { region: string; redirectUri: string; clientId?: string; clientSecret?: string; displayName?: string; environment?: string; integrationId?: string }) => ({ region: String(input.region ?? "").trim(), redirectUri: String(input.redirectUri ?? "").trim(), clientId: String(input.clientId ?? "").trim(), clientSecret: String(input.clientSecret ?? "").trim(), displayName: String(input.displayName ?? "").trim(), environment: String(input.environment ?? "Production").trim() || "Production", integrationId: String(input.integrationId ?? "").trim() })).handler(async ({ data, context }) => {
  ...
  try {
    ...
    await store.saveClientCredentials(integrationId, tenantId, credentials);
...
export const completeGenesysOAuth = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input: { code?: string; state?: string; error?: string }) => ({ code: input.code ? String(input.code) : "", state: input.state ? String(input.state) : "", error: input.error ? String(input.error) : "" })).handler(async ({ data, context }) => {
  ...
  try {
    ...
    const failExistingIntegration = async (error: unknown) => { const errorCode = errors.toErrorCode(error); const errorMessage = errors.toErrorMessage(error); await supabaseAdmin.from("integrations").update({ status: "failed", health_status: "unhealthy", health_detail: errorMessage, last_sync_error: errorMessage }).eq("id", integrationId).eq("tenant_id", tenantId); return { ok: false as const, errorCode, errorMessage }; };
    ...
    const credentials = await store.getStoredCredentials(integrationId, tenantId);
    const region = errors.normalizeGenesysRegion(oauthState.region);
    try {
      const tokens = await instanceOAuth.exchangeAuthorizationCode({ code: data.code, redirectUri: oauthState.redirect_uri, region, codeVerifier: oauthState.code_verifier ?? undefined, credentials: { clientId: credentials.clientId, clientSecret: credentials.clientSecret } });
...
export const verifyGenesysConnection = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input: { integrationId?: string }) => ({ integrationId: String(input?.integrationId ?? "").trim() })).handler(async ({ data, context }) => {
  ...
  try { const { tenantId } = await store.requireManage(context.supabase, context.userId); const { supabaseAdmin } = await import("@/integrations/supabase/client.server"); const { data: integration } = await supabaseAdmin.from("integrations").select("id,region").eq("id", data.integrationId).eq("tenant_id", tenantId).eq("provider", "genesys").maybeSingle(); if (!integration) throw new errors.IntegrationError("not_connected"); const credentials = await store.getStoredCredentials(integration.id, tenantId); if (!credentials.accessToken) throw new errors.IntegrationError("not_connected"); let accessToken = credentials.accessToken; if (credentials.expiresAt && new Date(credentials.expiresAt).getTime() - Date.now() < 120_000 && credentials.refreshToken) { const refreshed = await instanceOAuth.refreshAccessToken({ refreshToken: credentials.refreshToken, region: errors.normalizeGenesysRegion(integration.region), credentials: { clientId: credentials.clientId, clientSecret: credentials.clientSecret } }); accessToken = refreshed.accessToken; await store.saveTokens(integration.id, tenantId, refreshed); } const { org } = await instanceOAuth.healthCheck(accessToken, errors.normalizeGenesysRegion(integration.region)); await supabaseAdmin.from("integrations").update({ status: "connected", health_status: "healthy", health_detail: null, external_org_id: org.id, external_org_name: org.name }).eq("id", integration.id).eq("tenant_id", tenantId); return { ok: true as const, orgName: org.name, healthStatus: "healthy" }; }
  catch (error) { return { ok: false as const, errorCode: errors.toErrorCode(error), errorMessage: errors.toErrorMessage(error) }; }
});

export const syncGenesysNow = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input: { integrationId?: string }) => ({ integrationId: String(input?.integrationId ?? "").trim() })).handler(async ({ data, context }) => {
...
export const disconnectGenesys = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input: { integrationId?: string }) => ({ integrationId: String(input?.integrationId ?? "").trim() })).handler(async ({ data, context }) => {
  const store = await import("./genesys/store.server"); const errors = await import("./genesys/errors");
...
export const deleteGenesysIntegration = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input: { integrationId?: string }) => ({ integrationId: String(input?.integrationId ?? "").trim() })).handler(async ({ data, context }) => {
  const store = await import("./genesys/store.server"); const errors = await import("./genesys/errors");
  try {
    const { tenantId } = await store.requireManage(context.supabase, context.userId); if (!data.integrationId) throw new errors.IntegrationError("not_connected");
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server"); const { data: integration } = await supabaseAdmin.from("integrations").select("id,external_org_name").eq("id", data.integrationId).eq("tenant_id", tenantId).eq("provider", "genesys").maybeSingle(); if (!integration) throw new errors.IntegrationError("not_connected");
    const childTables = ["integration_oauth_states", "integration_sync_runs", "genesys_user_licenses", "genesys_users", "genesys_licenses", "genesys_queues"] as const;
    for (const table of childTables) { const { error } = await supabaseAdmin.from(table).delete().eq("integration_id", integration.id); if (error) throw new errors.IntegrationError("provider_error", error.message); }
    const { error: credentialError } = await (supabaseAdmin.from("provider_connections" as any) as any).delete().eq("integration_id", integration.id).eq("tenant_id", tenantId).eq("provider", "genesys"); if (credentialError) throw new errors.IntegrationError("provider_error", credentialError.message);
    const { error } = await supabaseAdmin.from("integrations").delete().eq("id", integration.id).eq("tenant_id", tenantId).eq("provider", "genesys"); if (error) throw new errors.IntegrationError("provider_error", error.message);
...
export const getGenesysSyncHistory = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((input: { integrationId?: string }) => ({ integrationId: String(input?.integrationId ?? "").trim() })).handler(async ({ data, context }) => {
