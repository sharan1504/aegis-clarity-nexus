...
export const completeGenesysOAuth = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      const integrationId = await store.ensureIntegration(tenantId, region, context.userId);
      const tokens = await connector.exchangeAuthorizationCode({ code: data.code, redirectUri, regionId: region });
      ...
      return { ok: false as const, errorCode: errors.toErrorCode(error), errorMessage: errors.toErrorMessage(error) };
    }
  });
...
export const verifyGenesysConnection = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    try {
      ...
      await store.markIntegration(summary.id, { status: "connected", health_status: "healthy", health_detail: null, external_org_id: org.id, external_org_name: org.name });
      await store.writeIntegrationAudit(context.supabase, { tenantId, action: "integration.verified", entityId: summary.id, detail: `Genesys connection verified against organization ${org.name}.`, payload: { provider: "genesys", orgId: org.id } });
...
export const syncGenesysNow = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    const store = await import("./genesys/store.server");
    const reconcile = await import("./genesys/reconcile.server");
    const errors = await import("./genesys/errors");
    try {
      const { tenantId } = await store.requireManage(context.supabase, context.userId);
      const summary = await store.getIntegrationSummary(context.supabase, tenantId);
      ...
      if (result.status === "success") {
        // The integration timestamp is the commit point for a successful full
        // sync. Only after success do we delete provider rows older than that
        // snapshot, which fixes stale/deleted users remaining in Aegis.
        const refreshed = await store.getIntegrationSummary(context.supabase, tenantId);
        if (refreshed?.lastSyncAt) {
          const deleted = await reconcile.reconcileGenesysSnapshot(summary.id, refreshed.lastSyncAt);
          return { ok: true as const, ...result, reconciled: deleted };
        }
      }
      return { ok: result.status === "success", ...result };
    } catch (error) {
      return { ok: false, status: "failed" as const, errorCode: errors.toErrorCode(error), errorMessage: errors.toErrorMessage(error) };
    }
  });
