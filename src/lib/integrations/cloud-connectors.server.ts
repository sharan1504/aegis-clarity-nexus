...
async function assumeAwsReadRole(credentials: AwsSyncCredentials, tenantId: string, connectionId: string) {
  ...
  return { accountId, accessKeyId: c.AccessKeyId, secretAccessKey: c.SecretAccessKey, sessionToken: c.SessionToken };
}

function assumedStsClient(session: Awaited<ReturnType<typeof assumeAwsReadRole>>) {
  return new STSClient({ region: AWS_REGION, credentials: { accessKeyId: session.accessKeyId, secretAccessKey: session.secretAccessKey, sessionToken: session.sessionToken } });
}
...
export async function syncAwsMinimal(credentials: AwsSyncCredentials, tenantId: string, connectionId: string) {
  ...
  return { rows: [{ entityType: "account", entityKey: session.accountId, payload: { accountId: session.accountId, region: credentials.region ?? "us-east-1" } }], observedAt };
}
