...
export const syncGitHubFn = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input: { connectionId: string; entityScope?: GitHubEntityScope }) => ({ connectionId: String(input.connectionId), entityScope: input.entityScope ?? "all" })).handler(async ({ data, context }) => {
  ...
  const { data: run, error } = await (context.supabase.from("provider_sync_runs") as unknown as { insert: (row: Record<string, unknown>) => any }).insert({ tenant_id: tenantId, provider: "github", connection_id: data.connectionId, idempotency_key: idempotencyKey, status: "running", started_at: new Date().toISOString() }).select("id").single();
