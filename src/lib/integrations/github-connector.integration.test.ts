    ...
    from(table: string) {
      const chain: any = {
        ...
        then: (resolve: (value: unknown) => unknown) => {
          if (table === "github_synced_entities") return Promise.resolve({ data: state.entities.map((row) => ({ entity_type: row.entity_type, entity_key: row.entity_key })), error: null }).then(resolve);
          if (table === "provider_sync_runs") return Promise.resolve({ data: null, error: null }).then(resolve);
...
const encrypted = (token: string) => {
  const key = Buffer.alloc(32, 7); process.env.AEGIS_CREDENTIAL_ENCRYPTION_KEY = key.toString("hex");
  const iv = crypto.randomBytes(12); const cipher = crypto.createCipheriv("aes-256-gcm", key, iv); const body = Buffer.concat([cipher.update(JSON.stringify({ accessToken: token }), "utf8"), cipher.final()]); const tag = cipher.getAuthTag(); return [iv.toString("base64url"), tag.toString("base64url"), body.toString("base64url")].join(".");
};
...
describe("GitHub sync end-to-end persistence", () => {
  it("stores repository, workflow and security records from a successful provider snapshot", async () => {
    ...
    expect(state.entities.map((row) => row.entity_type)).toEqual(["repository", "workflow_run", "security_alert", "security_alert"]);
  });
});
