...
type GitHubIssueAction = { type: "github.create_issue"; connectionId: string; owner: string; repo: string; title: string; body?: string; labels?: string[] };
type ExecutionState = { status: "proposed" | "executing" | "verified" | "failed" | "verification_unsupported"; action?: GitHubIssueAction; proposedAt?: string; startedAt?: string; completedAt?: string; verification?: Record<string, unknown>; sync?: Record<string, unknown>; error?: string };

function decryptCredentials(value: string): Record<string, unknown> { const keyHex = process.env.AEGIS_CREDENTIAL_ENCRYPTION_KEY; if (!keyHex || !/^[0-9a-fA-F]{64}$/.test(keyHex)) throw new Error("Credential encryption is not configured on the server."); const [iv, tag, ciphertext] = value.split("."); if (!iv || !tag || !ciphertext) throw new Error("GitHub credential ciphertext is malformed."); const decipher = crypto.createDecipheriv("aes-256-gcm", Buffer.from(keyHex, "hex"), Buffer.from(iv, "base64url")); decipher.setAuthTag(Buffer.from(tag, "base64url")); return JSON.parse(Buffer.concat([decipher.update(Buffer.from(ciphertext, "base64url")), decipher.final()]).toString("utf8")) as Record<string, unknown>; }
async function githubRequest<T>(path: string, token: string, init: RequestInit = {}): Promise<T> { const response = await fetch(`${API}${path}`, { ...init, headers: { accept: "application/vnd.github+json", authorization: `Bearer ${token}`, "x-github-api-version": API_VERSION, "user-agent": "Aegis-AI/1.0", ...(init.body ? { "content-type": "application/json" } : {}), ...(init.headers ?? {}) } }); const text = await response.text(); let body: unknown = {}; try { body = text ? JSON.parse(text) : {}; } catch { body = { raw: text }; } if (!response.ok) throw new Error(`GitHub API ${response.status}: ${typeof body === "object" ? JSON.stringify(body).slice(0, 1200) : String(body)}`); return body as T; }
async function loadGitHubConnection(tenantId: string, connectionId: string) { const { data, error } = await supabaseAdmin.from("provider_connections").select("id,tenant_id,provider,status,encrypted_credentials").eq("id", connectionId).eq("tenant_id", tenantId).eq("provider", "github").maybeSingle(); if (error) throw error; if (!data || data.status !== "connected" || !data.encrypted_credentials) throw new Error("The approved GitHub connection is not connected or has no server-side credentials."); const credentials = decryptCredentials(data.encrypted_credentials); return { ...data, credentials }; }
function safeRepoPart(value: string): string { if (!/^[A-Za-z0-9_.-]{1,100}$/.test(value)) throw new Error("The approved GitHub repository target is invalid."); return value; }
async function executeGitHubCreateIssue(tenantId: string, action: GitHubIssueAction) { const connection = await loadGitHubConnection(tenantId, action.connectionId); const token = await installationTokenForCredentials(connection.credentials); const owner = safeRepoPart(action.owner); const repo = safeRepoPart(action.repo); const created = await githubRequest<{ number: number; id: number; html_url: string; title: string; body: string | null; state: string; updated_at: string }>(`/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/issues`, token, { method: "POST", body: JSON.stringify({ title: action.title, ...(action.body ? { body: action.body } : {}), ...(action.labels?.length ? { labels: action.labels } : {}) }) }); const verified = await githubRequest<{ number: number; id: number; html_url: string; title: string; body: string | null; state: string; updated_at: string }>(`/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/issues/${created.number}`, token); if (verified.number !== created.number || verified.title !== created.title || verified.state !== "open") throw new Error("GitHub issue verification did not match the provider response."); return { kind: "github.issue", issueNumber: verified.number, issueId: verified.id, url: verified.html_url, title: verified.title, state: verified.state, verifiedAt: new Date().toISOString(), owner, repo }; }
async function updateChangeExecution(supabase: UserClient, tenantId: string, changeRecordId: string, state: ExecutionState, stage?: string, timelineText?: string) { const { data: current, error: loadError } = await (supabase as any).from("change_records").select("timeline").eq("id", changeRecordId).eq("tenant_id", tenantId).single(); if (loadError) throw loadError; const timeline = Array.isArray(current?.timeline) ? current.timeline : []; const event = timelineText ? [{ ts: new Date().toISOString(), actor: "Aegis governed execution", kind: "action", text: timelineText }, ...timeline] : timeline; const { error } = await (supabase as any).from("change_records").update({ execution: state, ...(stage ? { stage } : {}), timeline: event }).eq("id", changeRecordId).eq("tenant_id", tenantId); if (error) throw error; }
...
export async function executeApprovedAction(supabase: UserClient, actor: ActorContext, changeRecordId: string) {
  ...
  await updateChangeExecution(supabase, actor.tenantId, change.id, executing, "Executing", `Stage → Executing. Approved GitHub issue action started for ${change.change_id}.`);
  ...
  if (!governed.ok) {
    ...
    await updateChangeExecution(supabase, actor.tenantId, change.id, failed, "Failed", `Stage → Failed. GitHub execution was denied before provider mutation: ${failed.error}`);
  ...
  try {
    ...
    await updateChangeExecution(supabase, actor.tenantId, change.id, verified, "Verified", `Stage → Verified. GitHub issue mutation was provider-verified and post-change sync completed.`);
    ...
    await updateChangeExecution(supabase, actor.tenantId, change.id, failed, "Failed", `Stage → Failed. GitHub mutation occurred but verification or post-change sync failed: ${message}`);
