...
export function ProviderLogo({ provider, className = "h-10 w-10" }: { provider: CatalogProvider; className?: string }) {
  ...
  return (
    <div className={`flex shrink-0 items-center justify-center overflow-hidden rounded-lg border bg-background p-2 ${className}`}>
      {!failed ? (
        <img
...
export function ProviderAvailabilityBadge({ provider }: { provider: CatalogProvider }) {
  return provider.availability === "available" ? (
    ...
    <Badge variant="outline">
      <Clock3 className="mr-1 h-3 w-3" />Coming soon
    </Badge>
  );
}
...
export function ProviderContractHint({ provider }: { provider: CatalogProvider }) {
  return (
    <span className="text-xs text-muted-foreground">
      {provider.capabilities.includes("read") && provider.capabilities.includes("sync") ? "Read / sync" : "Catalog only"}
    </span>
  );
}

export function CatalogSearch({ value, onChange, placeholder = "Search integrations" }: {
  value: string;
...
export function filterProviders(providers: CatalogProvider[], query: string, category: string) {
  ...
  return providers.filter(
    (provider) =>
      (!q || [provider.name, provider.category, provider.description].some((value) => value.toLowerCase().includes(q))) &&
    ...
    <Card className="flex h-full flex-col transition-colors hover:border-primary/40">
      <CardHeader className="space-y-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap justify-end gap-2">
            <Badge variant="secondary">Installed {installedCount}/{MAX_PROVIDER_INSTANCES}</Badge>
            <ProviderAvailabilityBadge provider={provider} />
          </div>
        </div>
        <div>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2">
            ...
            <ProviderContractHint provider={provider} />
          </div>
        </div>
      </CardHeader>
      <CardContent className="flex-1">
        ...
        <div className="mt-4 text-xs text-muted-foreground">
          Auth: <span className="font-medium text-foreground">{provider.auth}</span>
        </div>
      </CardContent>
      <CardFooter className="flex flex-wrap gap-2 border-t pt-4">
        <Button asChild size="sm" variant="outline">
          <Link to="/integrations/catalog/$providerId" params={{ providerId: provider.id }}>Details</Link>
        </Button>
        <Button asChild size="sm" variant="outline">
          <Link to="/help" search={{ topic: `provider-${provider.id}` }}>Setup guide</Link>
        </Button>
        {available && !atCap ? (
          <Button
            type="button"
          ...
          <Button size="sm" variant="outline" disabled>Limit reached</Button>
        ) : (
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <CatalogSearch value={query} onChange={setQuery} placeholder="Search by provider, category, or description" />
      ...
      {filtered.length ? (
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
          {filtered.map((provider) => <ProviderCatalogCard key={provider.id} provider={provider} />)}
        </div>
      ) : (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Try a different provider, category, or search term.</p>
          </CardContent>
        </Card>
      )}
    </div>
  ...
  return (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      <div className="flex items-start gap-4">
        <ProviderLogo provider={provider} className="h-14 w-14" />
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h1 className="text-2xl font-semibold tracking-tight">{provider.name}</h1>
            <ProviderAvailabilityBadge provider={provider} />
            <Badge variant="secondary">Installed {provider.connections.length}/{MAX_PROVIDER_INSTANCES}</Badge>
          </div>
          <div className="mt-2 flex flex-wrap gap-2">
            <Badge variant="secondary">{provider.category}</Badge>
            <Badge variant="outline">{provider.auth}</Badge>
            <ProviderContractHint provider={provider} />
          </div>
      ...
      <Button asChild variant="outline">
        <Link to="/integrations/catalog"><ArrowLeft className="mr-2 h-4 w-4" />Back to catalog</Link>
      </Button>
    </div>
  );
}
...
export function CapabilityList({ provider }: { provider: CatalogProvider }) {
  return (
    <div className="grid gap-3 sm:grid-cols-3">
      {["read", "sync", "write"].map((capability) => {
        const enabled = provider.capabilities.includes(capability as never);
        return (
          <div key={capability} className="rounded-lg border p-3">
            ...
            <p className="mt-1 text-xs text-muted-foreground">{enabled ? "Declared capability" : "Not declared"}</p>
          </div>
        );
      })}
    </div>
