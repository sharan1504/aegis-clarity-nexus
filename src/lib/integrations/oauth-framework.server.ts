...
): Promise<OAuthStateRecord & { codeVerifier?: string }> {
  if (!state.trim()) throw new Error("OAuth state is required.");
  const { data, error } = await db.from("integration_oauth_states")
...
export function assertFreshExpiry(expiresAt: string | null | undefined, skewMs = 120_000): boolean {
...
export async function readConnectionCredentials<T>(db: AdminClient, connectionId: string, tenantId: string): Promise<T> {
  const { data, error } = await db.from("provider_connections")
    .select("encrypted_credentials")
    .eq("id", connectionId)
    .eq("tenant_id", tenantId)
    .maybeSingle();
  if (error) throw new Error(`Unable to read integration credentials: ${error.message}`);
  if (!data?.encrypted_credentials) throw new Error("Integration credentials are missing. Please reconnect.");
  return decryptCredentials<T>(data.encrypted_credentials);
}

export async function markReconnectRequired(db: AdminClient, connectionId: string, message = "Reconnect required.") {
  await db.from("provider_connections").update({ status: "failed", last_error: message, updated_at: new Date().toISOString() }).eq("id", connectionId);
}
