...
async function tokenRequest(input: { accountUrl: string; clientId: string; clientSecret: string; body: URLSearchParams }) {
  const response = await fetch(`${accountUrl(input.accountUrl)}/oauth/token-request`, { method: "POST", headers: { authorization: `Basic ${Buffer.from(`${encodeURIComponent(input.clientId)}:${encodeURIComponent(input.clientSecret)}`).toString("base64")}`, "content-type": "application/x-www-form-urlencoded", accept: "application/json" }, body: input.body });
...
export async function startSnowflakeOAuth(input: { tenantId: string; connectionId?: string; clientId: string; clientSecret: string; accountUrl: string; redirectUri: string; displayName?: string; environment?: string; scope?: string }) {
  ...
  const credentials: SnowflakeCredentials = {
    ...
    accountUrl: normalized,
    displayName: input.displayName ?? "Snowflake",
    environment: input.environment ?? "Production",
  };
  await db.from("provider_connections").upsert({ id: connectionId, tenant_id: input.tenantId, provider: "snowflake", display_name: credentials.displayName, environment: credentials.environment, status: "failed", encrypted_credentials: encryptCredentials(credentials), last_error: "OAuth authorization in progress", updated_at: new Date().toISOString() }, { onConflict: "id" });
  ...
  return { connectionId, authorizeUrl: buildSnowflakeAuthorizeUrl({ accountUrl: normalized, clientId: credentials.clientId, redirectUri: input.redirectUri, state, scope: input.scope, codeChallenge: challenge }) };
}
...
export async function completeSnowflakeOAuth(state: string, code: string) {
  ...
  const credentials = await readConnectionCredentials<SnowflakeCredentials>(db, stateRecord.connectionId, stateRecord.tenantId);
  ...
  await storeOAuthConnection(db, { connectionId: stateRecord.connectionId, tenantId: stateRecord.tenantId, provider: "snowflake", externalId: new URL(credentials.accountUrl).hostname, displayName: credentials.displayName ?? "Snowflake", environment: credentials.environment ?? "Production", credentials: { ...credentials, accessToken: token.access_token, refreshToken: token.refresh_token, accessTokenExpiresAt: expiresAt, scopes: token.scope?.split(/\s+/).filter(Boolean) ?? [...SNOWFLAKE_SCOPES] }, expiresAt });
  return { connectionId: stateRecord.connectionId, displayName: credentials.displayName ?? "Snowflake" };
}

export async function ensureSnowflakeAccessToken(connectionId: string, tenantId: string) {
  const db = await getAdminClient();
  const credentials = await readConnectionCredentials<SnowflakeCredentials>(db, connectionId, tenantId);
  if (credentials.accessToken && credentials.accessTokenExpiresAt && new Date(credentials.accessTokenExpiresAt).getTime() - Date.now() > 60_000) return credentials.accessToken;
  ...
  try {
    ...
    const expiresAt = new Date(Date.now() + Number(token.expires_in ?? 600) * 1000).toISOString();
    await storeOAuthConnection(db, { connectionId, tenantId, provider: "snowflake", externalId: new URL(credentials.accountUrl).hostname, displayName: credentials.displayName ?? "Snowflake", environment: credentials.environment ?? "Production", credentials: { ...credentials, accessToken: token.access_token, refreshToken: token.refresh_token ?? credentials.refreshToken, accessTokenExpiresAt: expiresAt }, expiresAt });
