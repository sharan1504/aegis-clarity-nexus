...
type WorkdayCredentials = { region: WorkdayRegion; tenantAlias: string; clientId: string; clientSecret: string; accessToken?: string; refreshToken?: string; expiresAt?: string; displayName?: string };
...
function authBase(region: WorkdayRegion) { return WORKDAY_AUTH_BASE_URLS[region]; }

async function tokenRequest(region: WorkdayRegion, body: URLSearchParams) {
  const response = await fetch(`${authBase(region)}/v1/token`, { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded", accept: "application/json" }, body });
...
async function validateWorkdayAccess(credentials: WorkdayCredentials, accessToken: string) {
  const response = await fetch("https://api.workday.com/common/v1/workers?limit=1", { headers: { authorization: `Bearer ${accessToken}`, accept: "application/json" } });
  const text = await response.text();
...
export function buildWorkdayAuthorizeUrl(input: { region: WorkdayRegion; clientId: string; redirectUri: string; state: string; challenge: string }) {
  ...
  url.searchParams.set("response_type", "code"); url.searchParams.set("client_id", input.clientId.trim()); url.searchParams.set("redirect_uri", input.redirectUri); url.searchParams.set("state", input.state); url.searchParams.set("code_challenge", input.challenge); url.searchParams.set("code_challenge_method", "S256");
  return url.toString();
}

async function exchangeCode(input: { region: WorkdayRegion; code: string; clientId: string; redirectUri: string; verifier: string }) {
  return tokenRequest(input.region, form({ grant_type: "authorization_code", code: input.code, client_id: input.clientId, code_verifier: input.verifier, redirect_uri: input.redirectUri }));
}
...
export async function startWorkdayOAuth(input: { tenantId: string; connectionId?: string; region: WorkdayRegion; tenantAlias: string; clientId: string; clientSecret: string; redirectUri: string; displayName?: string; environment?: string }) {
  const db = await getAdminClient(); const tenantAlias = normalizeTenantAlias(input.tenantAlias); const connectionId = input.connectionId ?? crypto.randomUUID(); const { verifier, challenge } = createPkcePair();
...
export async function completeWorkdayOAuth(state: string, code: string) {
  const db = await getAdminClient(); const stateRecord = await consumeOAuthState(db, state, "workday"); const credentials = await readConnectionCredentials<WorkdayCredentials>(db, stateRecord.connectionId, stateRecord.tenantId);
  if (!stateRecord.codeVerifier) throw new Error("Workday PKCE verifier is missing. Please reconnect.");
  const tokens = await exchangeCode({ region: credentials.region, code, clientId: credentials.clientId, redirectUri: stateRecord.redirectUri, verifier: stateRecord.codeVerifier }); await validateWorkdayAccess(credentials, tokens.access_token);
  const expiresAt = new Date(Date.now() + Number(tokens.expires_in ?? 3600) * 1000).toISOString(); const displayName = credentials.displayName ?? `Workday (${credentials.tenantAlias})`;
...
export async function ensureWorkdayAccessToken(connectionId: string, tenantId: string) {
  const db = await getAdminClient(); const credentials = await readConnectionCredentials<WorkdayCredentials>(db, connectionId, tenantId);
  ...
  const tokens = await tokenRequest(credentials.region, form({ grant_type: "refresh_token", refresh_token: credentials.refreshToken })); const expiresAt = new Date(Date.now() + Number(tokens.expires_in ?? 3600) * 1000).toISOString(); await validateWorkdayAccess(credentials, tokens.access_token);
  await storeOAuthConnection(db, { connectionId, tenantId, provider: "workday", externalId: credentials.tenantAlias, displayName: credentials.displayName ?? `Workday (${credentials.tenantAlias})`, credentials: { ...credentials, accessToken: tokens.access_token, refreshToken: tokens.refresh_token ?? credentials.refreshToken, expiresAt }, expiresAt }); return tokens.access_token;
}
