...
export const ZENDESK_SCOPES = ["read", "tickets:read", "users:read", "organizations:read"] as const;
...
function origin(subdomain: string) { return `https://${subdomain.trim()}.zendesk.com`; }
function form(data: Record<string, string>) { return new URLSearchParams(data); }

export function buildZendeskAuthorizeUrl(input: { subdomain: string; clientId: string; redirectUri: string; state: string }) {
...
async function tokenRequest(subdomain: string, body: URLSearchParams) {
  const response = await fetch(`${origin(subdomain)}/oauth/tokens`, { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded" }, body });
...
export async function startZendeskOAuth(input: { tenantId: string; connectionId?: string; subdomain: string; clientId: string; clientSecret: string; redirectUri: string; displayName?: string; environment?: string }) {
  const subdomain = input.subdomain.trim().replace(/^https?:\/\//, "").split(".")[0];
  if (!/^[a-zA-Z0-9-]+$/.test(subdomain)) throw new Error("Zendesk subdomain is invalid.");
  const db = await getAdminClient();
  const connectionId = input.connectionId ?? crypto.randomUUID();
  await storeOAuthConnection(db, { connectionId, tenantId: input.tenantId, provider: "zendesk", displayName: input.displayName ?? `Zendesk (${subdomain})`, environment: input.environment ?? "Production", status: "failed", credentials: { provider: "zendesk", clientId: input.clientId.trim(), clientSecret: input.clientSecret, subdomain, displayName: input.displayName }, expiresAt: null });
  ...
  return { connectionId, authorizeUrl: buildZendeskAuthorizeUrl({ subdomain, clientId: input.clientId, redirectUri: input.redirectUri, state }) };
}
...
export async function ensureZendeskAccessToken(connectionId: string, tenantId: string) {
  ...
  const tokens = await tokenRequest(credentials.subdomain, form({ grant_type: "refresh_token", refresh_token: credentials.refreshToken, client_id: credentials.clientId, client_secret: credentials.clientSecret }));
  const expiresAt = new Date(Date.now() + Number(tokens.expires_in ?? 1800) * 1000).toISOString();
  await storeOAuthConnection(db, { connectionId, tenantId, provider: "zendesk", externalId: credentials.userId ?? null, displayName: credentials.displayName ?? `Zendesk (${credentials.subdomain})`, credentials: { ...credentials, accessToken: tokens.access_token, refreshToken: tokens.refresh_token ?? credentials.refreshToken, expiresAt, scopes: tokens.scope?.split(" ") ?? credentials.scopes }, expiresAt });
