...
import { startNewRelicConnection } from "./oauth-newrelic.server";
...
export const startNewRelicConnection = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input: { connectionId?: string; apiKey: string; region: "us" | "eu" | "jp"; displayName?: string; environment?: string }) => input).handler(async ({ data, context }) => startNewRelicConnection({ data, context } as any));
