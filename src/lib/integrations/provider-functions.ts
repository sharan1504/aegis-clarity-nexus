...
function encryptCredentials(value: unknown): string {
  ...
  const key = Buffer.from(keyHex, "hex"); const iv = crypto.randomBytes(12); const cipher = crypto.createCipheriv("aes-256-gcm", key, iv);
...
type CatalogConnection = {
  id: string; provider: string; status: "connected" | "failed" | "disconnected"; display_name: string | null; environment: string; external_id: string | null;
  credential_expires_at: string | null; last_sync_at: string | null; last_error: string | null; connected_at: string | null; updated_at: string;
};
...
export const startProviderConnection = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => { const provider = PROVIDER_REGISTRY.find((p) => p.id === data.provider); if (!provider) return { ok: false as const, errorCode: "provider_not_supported", errorMessage: "Provider is not registered." }; return { ok: true as const, provider, requiresCredentials: true }; });
...
export const connectProvider = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    try {
      ...
      const displayName = data.displayName?.trim() || result.displayName || null; const environment = data.environment?.trim() || "Production";
      const { data: connectionId, error } = await context.supabase.rpc("upsert_provider_connection", { p_tenant_id: tenantId, p_connection_id: data.connectionId ?? null, p_provider: data.provider, p_external_id: result.externalId ?? null, p_display_name: displayName, p_environment: environment, p_status: result.status, p_encrypted_credentials: encrypted, p_credential_expires_at: result.expiresAt ?? null, p_last_error: result.error ?? null });
    ...
    } catch (error) { return { ok: false as const, error: error instanceof Error ? error.message : "Connection failed." }; }
  });
