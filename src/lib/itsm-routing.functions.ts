...
export const discoverItsmRouting = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input: { integrationId: string; provider: "jira" | "servicenow" }) => input).handler(async ({ data, context }) => {
  ...
  const { data: integration, error } = await context.supabase.from("integrations").select("id,tenant_id,provider,status,is_mock").eq("id", data.integrationId).eq("tenant_id", tenantId).maybeSingle();
  ...
  if (data.provider === "jira") {
    ...
    const site = Array.isArray(resources) ? resources.find((item: any) => item.id) : null; const cloudId = String(site?.id ?? "");
    ...
    const [projects, issueTypes] = await Promise.all([requestJson(`${root}/rest/api/3/project/search?maxResults=200`, credentials.accessToken), requestJson(`${root}/rest/api/3/issuetype`, credentials.accessToken)]);
  ...
  const baseUrl = credentials.baseUrl?.replace(/\/$/, ""); if (!baseUrl) throw new Error("ServiceNow connection is missing its instance URL.");
  const [groups, categories] = await Promise.all([requestJson(`${baseUrl}/api/now/table/sys_user_group?sysparm_fields=sys_id,name&sysparm_limit=200&sysparm_query=active=true^ORDERBYname`, credentials.accessToken), requestJson(`${baseUrl}/api/now/table/sys_choice?sysparm_fields=value,label&sysparm_limit=200&sysparm_query=name=change_request^element=category^inactive=false^ORDERBYlabel`, credentials.accessToken)]);
