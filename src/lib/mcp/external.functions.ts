...
import { encryptCredentials } from "@/lib/integrations/credential-vault.server";
...
export const listMcpServers = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).handler(async ({ context }) => {
  const tenant = await resolveTenantContext(context.supabase, context.userId);
  const result = await (context.supabase as any).from("mcp_server_connections")
    .select("id,display_name,base_url,auth_type,status,last_discovered_at,last_error,metadata,created_at,updated_at")
    .eq("tenant_id", tenant.tenantId).order("display_name");
...
export const connectMcpServer = createServerFn({ method: "POST" })
  ...
  .inputValidator((input: { displayName: string; baseUrl: string; token?: string }) => ({
  ...
  .handler(async ({ data, context }) => {
    ...
    const encrypted = token ? encryptCredentials({ token }) : null;
...
export const discoverMcpServerTools = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { serverId: string }) => ({ serverId: String(input?.serverId ?? "").trim() }))
  .handler(async ({ data, context }) => {
    const tenant = await requireManager(context);
    if (!data.serverId) throw new Error("An MCP server is required.");
    const server = await (context.supabase as any).from("mcp_server_connections")
      .select("id,base_url,auth_type,encrypted_auth,status").eq("id",data.serverId).eq("tenant_id",tenant.tenantId).maybeSingle();
...
export const bindExternalMcpToolToAgent = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { agentKey: string; toolName: string }) => ({ agentKey:String(input?.agentKey ?? "").trim(), toolName:String(input?.toolName ?? "").trim() }))
  .handler(async ({ data, context }) => {
    const tenant = await requireManager(context);
    const tool = await (context.supabase as any).from("mcp_tool_definitions").select("id,origin,enabled,capability_key").eq("tool_name",data.toolName).eq("origin","external").eq("enabled",true).maybeSingle();
