...
type GovernedMcpContext = { isAuthenticated: () => boolean; token?: string; userId?: string };

export default defineTool({
  name: "propose_change_record",
  title: "Propose a change record",
  description: "Draft a tenant-scoped change record for human review. This tool can only create a Proposed record with pending approval; it cannot approve, execute, or select an execution mode.",
  inputSchema: {
  ...
  handler: async (input, rawCtx) => {
    const ctx = rawCtx as unknown as GovernedMcpContext;
    if (!ctx.isAuthenticated() || !ctx.token || !ctx.userId) return { content: [{ type: "text", text: "Not authenticated" }], isError: true };
    ...
    return { content: [{ type: "text", text: JSON.stringify({ ...result, approvalStatus: "pending", executionStatus: "not_executed" }, null, 2) }], structuredContent: { ...result, approvalStatus: "pending", executionStatus: "not_executed" } };
  },
});
