...
import { ensureSalesforceAccessToken, withSalesforceAccessToken } from "@/lib/integrations/oauth-salesforce.server";
...
async function json(url: string, token: string, init: RequestInit = {}) {
  ...
  if (!response.ok) { const error = new Error(`Provider request failed (${response.status}): ${JSON.stringify(body).slice(0, 1000)}`); (error as Error & { status?: number }).status = response.status; throw error; }
  return body;
}
...
async function fetchProvider(provider: Exclude<Provider, "github">, credentials: Credentials, connectionId: string, tenantId: string) {
  if (provider === "m365") {
    ...
    const result = await new Microsoft365LicenseConnector({ tenantId: credentials.tenant, clientId: credentials.clientId, clientSecret: credentials.clientSecret }).sync();
    return [
  ...
  if (provider === "jira") token = await ensureJiraAccessToken(connectionId, tenantId);
  if (provider === "salesforce") token = await ensureSalesforceAccessToken(connectionId, tenantId);
  if (provider === "servicenow") token = await ensureServiceNowAccessToken(connectionId, tenantId);
  if (provider === "slack") token = await ensureSlackAccessToken(connectionId, tenantId);
  if (!token) throw new Error(`${provider} has no access token.`);
  if (provider === "slack") {
    ...
    return [{ entityType: "workspace", entityKey: String(auth.team_id), payload: { team: auth.team, teamId: auth.team_id, userId: auth.user_id } }, ...((channels.channels ?? []) as any[]).map((c) => ({ entityType: "channel", entityKey: String(c.id), payload: { name: c.name, isPrivate: c.is_private, memberCount: c.num_members, topic: c.topic?.value ?? "", purpose: c.purpose?.value ?? "" } }))];
  }
  if (provider === "salesforce") {
    const instance = credentials.instanceUrl;
    if (!instance) throw new Error("Salesforce instance URL is missing from the OAuth token response. Reconnect required.");
    return withSalesforceAccessToken(connectionId, tenantId, async (salesforceToken) => {
      const org = await json(`${instance.replace(/\/$/, "")}/services/data/v66.0/connect/organization`, salesforceToken);
      return [{ entityType: "organization", entityKey: String(org.id ?? tenantId), payload: { organizationName: org.organizationName ?? "Salesforce", instanceUrl: instance } }];
    });
  }
  if (provider === "servicenow") {
    const instance = credentials.instanceUrl;
    if (!instance) throw new Error("ServiceNow instance URL is missing. Reconnect required.");
    const me = await json(`${instance.replace(/\/$/, "")}/api/now/table/sys_user?sysparm_limit=20&sysparm_fields=sys_id,name,user_name,email,active`, token);
    return ((me.result ?? []) as any[]).map((u) => ({ entityType: "user", entityKey: String(u.sys_id), payload: { name: u.name, userName: u.user_name, email: u.email, active: u.active } }));
  }
  const sites = await json("https://api.atlassian.com/oauth/token/accessible-resources", token);
  const site = Array.isArray(sites) ? sites.find((item: any) => item.id) : null;
  if (!site?.id) throw new Error("Jira returned no accessible site.");
  const base = `https://api.atlassian.com/ex/jira/${encodeURIComponent(credentials.cloudId ?? site.id)}`;
  ...
  return [...projectRows, ...(issues.issues ?? []).map((i: any) => ({ entityType: "issue", entityKey: String(i.id), payload: { key: i.key, summary: i.fields?.summary, status: i.fields?.status?.name, issueType: i.fields?.issuetype?.name, project: i.fields?.project?.key, assignee: i.fields?.assignee?.displayName ?? null, updated: i.fields?.updated, created: i.fields?.created } }))];
}

async function reconcileProviderEntities(supabaseAdmin: any, tenantId: string, provider: string, connectionId: string, currentRows: Array<{ entityType: string; entityKey: string }>, observedAt: string) {
  const { data: existing, error } = await supabaseAdmin.from("provider_sync_entities").select("entity_type,entity_key").eq("tenant_id", tenantId).eq("provider", provider).eq("connection_id", connectionId).eq("stale", false);
  if (error) throw error;
  const currentKeys = new Set(currentRows.map((row) => `${row.entityType}:${row.entityKey}`));
  const staleRows = (existing ?? []).filter((row: any) => !currentKeys.has(`${row.entity_type}:${row.entity_key}`));
  for (const row of staleRows) {
    const { error: staleError } = await supabaseAdmin.from("provider_sync_entities").update({ stale: true, observed_at: observedAt }).eq("tenant_id", tenantId).eq("provider", provider).eq("connection_id", connectionId).eq("entity_type", row.entity_type).eq("entity_key", row.entity_key);
    if (staleError) throw staleError;
  }
  return staleRows.length;
}

async function updateConnectionEvidence(supabaseAdmin: any, tenantId: string, connectionId: string, values: Record<string, unknown>) {
  const { error } = await supabaseAdmin.from("provider_connections").update({ ...values, updated_at: new Date().toISOString() }).eq("id", connectionId).eq("tenant_id", tenantId);
  if (error) throw error;
}
...
export const syncReportProvider = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input: { provider: Provider; connectionId?: string }) => ({ provider: input.provider, connectionId: input.connectionId ? String(input.connectionId).trim() : null })).handler(async ({ data, context }) => {
  ...
  try {
    ...
    const rows = await fetchProvider(data.provider, credentials, connection.id, tenantId);
    const observedAt = new Date().toISOString();
    const staleCount = await reconcileProviderEntities(supabaseAdmin, tenantId, data.provider, connection.id, rows, observedAt);
    for (const row of rows) {
      const { error } = await supabaseAdmin.from("provider_sync_entities").upsert({ tenant_id: tenantId, provider: data.provider, connection_id: connection.id, entity_type: row.entityType, entity_key: row.entityKey, payload: row.payload, observed_at: observedAt, sync_run_id: run.id, stale: false }, { onConflict: "tenant_id,provider,connection_id,entity_type,entity_key" });
      if (error) throw error;
    }
    const finished = new Date().toISOString();
    await supabaseAdmin.from("provider_sync_runs").update({ status: "success", finished_at: finished, records_seen: rows.length, records_upserted: rows.length, records_staled: staleCount }).eq("id", run.id).eq("tenant_id", tenantId);
    ...
    clearEvidenceCache();
    return { ok: true as const, provider: data.provider, connectionId: connection.id, records: rows.length, stale: staleCount, finishedAt: finished };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    await supabaseAdmin.from("provider_sync_runs").update({ status: "failed", finished_at: new Date().toISOString(), error_message: message.slice(0, 2000) }).eq("id", run.id).eq("tenant_id", tenantId);
    await updateConnectionEvidence(supabaseAdmin, tenantId, connection.id, { status: "failed", health_status: "unhealthy", health_checked_at: new Date().toISOString(), health_error: message.slice(0, 2000), last_sync_status: "failed", sync_error: message.slice(0, 2000), last_error: message.slice(0, 2000) });
    await recordOperationalIssueSafely(supabaseAdmin as any, { tenantId, source: "sync", severity: "high", title: `${data.provider} provider sync failed`, detail: `Provider synchronization failed for connection ${connection.id}. ${message}`, relatedId: run.id });
...
export async function loadProviderReportData(supabase: any, userId: string, departmentKey?: string | null) {
  ...
  return withEvidenceCache(tenantId, "provider-report", scope, async () => {
    const [{ data: connections }, allowedProviders] = await Promise.all([supabase.from("provider_connections").select("id,provider,status,display_name,last_sync_at").eq("tenant_id", tenantId), getDepartmentProviders(supabase, department)]);
    ...
    const { data: entityData } = providerNames.length ? await supabase.from("provider_sync_entities").select("provider,connection_id,entity_type,entity_key,payload,observed_at").eq("tenant_id", tenantId).eq("stale", false).in("provider", providerNames) : { data: [] };
    const { data: runData } = providerNames.length ? await supabase.from("provider_sync_runs").select("provider,connection_id,status,started_at,finished_at,records_seen,error_message").eq("tenant_id", tenantId).in("provider", providerNames).order("started_at", { ascending: false }).limit(100) : { data: [] };
    return { connectedProviders: scoped, entities: entityData ?? [], runs: runData ?? [], department: { key: department.departmentKey, name: department.departmentName, unrestricted: department.unrestricted } };
  });
}
...
export function deriveCorrelatedSignals(entities: any[]): CorrelatedSignal[] {
  ...
  for (const repo of github) for (const issue of jira) {
    ...
    signals.push({ title: `GitHub activity aligns temporally with Jira issue ${issue.payload.key}`, detail: `${repo.payload.name} was pushed near the time Jira issue ${issue.payload.key} was updated. This is a temporal correlation only; CenOps does not infer causation.`, providers: ["GitHub", "Jira"], timestamp: new Date(Math.max(repoAt, issueAt)).toISOString(), evidence: [{ provider: "GitHub", entityType: repo.entity_type, entityKey: repo.entity_key, observedAt: repo.observed_at }, { provider: "Jira", entityType: issue.entity_type, entityKey: issue.entity_key, observedAt: issue.observed_at }] });
