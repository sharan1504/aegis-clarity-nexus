...
export const listTenantUsers = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).handler(async ({ context }) => {
  ...
  const { data: members, error } = await context.supabase.from("profiles").select("id, tenant_id, email, full_name, organization, created_at, updated_at").eq("tenant_id", profile.tenant_id).order("full_name", { ascending: true });
  ...
  try {
    ...
    confirmed = new Set((data?.users ?? []).filter((u) => Boolean(u.email_confirmed_at)).map((u) => u.id));
  } catch {
    confirmed = new Set((members ?? []).map((m) => m.id));
  }

  return { tenantId: profile.tenant_id, members: (members ?? []).map((member) => ({ ...member, role: roleMap.get(member.id) ?? "viewer", status: confirmed.has(member.id) ? "active" as const : "invited" as const })) };
});

export const inviteTenantUser = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input: { fullName: string; email: string; organization: string; role: string }) => ({
  fullName: String(input.fullName ?? "").trim(), email: String(input.email ?? "").trim().toLowerCase(), organization: String(input.organization ?? "").trim(), role: String(input.role ?? "viewer").trim().toLowerCase(),
  ...
  const { data: inviter, error: inviterError } = await context.supabase.from("profiles").select("tenant_id, full_name").eq("id", context.userId).single();
  ...
  const { error: profileError } = await admin.from("profiles").upsert({ id: invited.user.id, tenant_id: inviter.tenant_id, email: data.email, full_name: data.fullName, organization: data.organization });
