...
export const listTenantUsers = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: members, error } = await context.supabase
      .from("profiles")
      .select("id, tenant_id, email, full_name, created_at, updated_at")
...
export const inviteTenantUser = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { fullName: string; email: string; organization: string; role: string }) => ({
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: inviter, error: inviterError } = await context.supabase
      .from("profiles")
    ...
    const { data: inviterRole } = await context.supabase
      .from("user_roles")
      .select("role")
    ...
    const { data: invited, error: inviteError } = await admin.auth.admin.inviteUserByEmail(data.email, {
      data: {
        ...
        invited_tenant_id: inviter.tenant_id,
    ...
    if (roleError && !roleError.message.toLowerCase().includes("duplicate")) {
      throw new Error(`Invitation sent, but the workspace role could not be created: ${roleError.message}`);
    }
