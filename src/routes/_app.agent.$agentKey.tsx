...
export const Route = createFileRoute("/_app/agent/$agentKey")({
  head: ({ params }) => pageHead({ path: `/agent/${params.agentKey}`, title: "Agent Configuration — Aegis AI", description: "Configure an agent's purpose, capabilities, policies and instructions." }),
...
function AgentDetailPage() {
  ...
  return <div>
    <PageHeader title={data.displayName} description={data.description ?? "Configure this agent for your organization's governed operations."} actions={<div className="flex gap-2"><Button variant="outline" asChild><Link to="/agents"><ArrowLeft className="mr-1.5 h-4 w-4" />Back to agents</Link></Button><Button asChild><Link to="/agentic-studio"><Sparkles className="mr-1.5 h-4 w-4" />Open Agentic Studio</Link></Button></div>} />
