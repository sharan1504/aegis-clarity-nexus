...
export const Route = createFileRoute("/_app/agents")({ head: () => pageHead({ path: "/agents", title: "AI Agents — Cenops", description: "Operate deployed agents and deploy verified agent definitions from one workspace." }), component: AgentsPage });
...
function AgentsPage() {
  const { tenantId } = useTenantContext(); const updateBinding = useServerFn(updateAgentDataSource); const deploy = useServerFn(deployAgent);
