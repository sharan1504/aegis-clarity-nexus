...
export function OverviewView({ data, max, onNavigate }: { data: Analytics; max: number; onNavigate: (target: AnalyticsDrillDown) => void }) {
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
        <button type="button" onClick={() => onNavigate({ view: "admin-activity" })} className="block w-full rounded-lg text-left transition-colors hover:ring-1 hover:ring-primary/30"><Metric icon={Activity} label="Audit events" value={data.platform.totalEvents.toLocaleString()} /></button>
        <button type="button" onClick={() => onNavigate({ view: "admin-activity" })} className="block w-full rounded-lg text-left transition-colors hover:ring-1 hover:ring-primary/30"><Metric icon={Users} label="Active actors" value={data.platform.activeUsers.toLocaleString()} /></button>
        <button type="button" onClick={() => onNavigate({ view: "governance" })} className="block w-full rounded-lg text-left transition-colors hover:ring-1 hover:ring-primary/30"><Metric icon={GitBranch} label="Change records" value={data.platform.changeRecords.toLocaleString()} /></button>
        <button type="button" onClick={() => onNavigate({ view: "ai-usage" })} className="block w-full rounded-lg text-left transition-colors hover:ring-1 hover:ring-primary/30"><Metric icon={Bot} label="AI requests" value={data.ai.requests.toLocaleString()} /></button>
        <button type="button" onClick={() => onNavigate({ view: "ai-usage" })} className="block w-full rounded-lg text-left transition-colors hover:ring-1 hover:ring-primary/30"><Metric icon={Clock3} label="Avg AI latency" value={`${data.ai.averageLatencyMs} ms`} /></button>
      </div>
...
export function AgentsView({ data, agent: agentKey, onNavigate }: { data: Analytics; agent?: string; onNavigate: (target: AnalyticsDrillDown) => void }) {
  ...
  return <div className="space-y-4">
    {focused ? <Card className="border-primary/30 bg-primary/[0.03] shadow-none"><CardHeader className="pb-2"><div className="flex items-start justify-between gap-3"><div><CardTitle className="text-base">{focused.name}</CardTitle><div className="mt-1 text-xs text-muted-foreground">{focused.category} · evidence-focused detail</div></div><button type="button" onClick={() => onNavigate({ view: "agents" })} className="text-xs text-muted-foreground hover:text-foreground">Clear focus</button></div></CardHeader><CardContent className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5"><MetricSmall label="Actions" value={focused.actions} /><MetricSmall label="Changes" value={focused.changes} /><MetricSmall label="Tokens" value={focused.tokens.toLocaleString()} /><MetricSmall label="Avg latency" value={focused.averageLatencyMs ? `${focused.averageLatencyMs} ms` : "—"} /><MetricSmall label="Last activity" value={focused.lastActivity ? new Date(focused.lastActivity).toLocaleString() : "No activity"} />{focused.recentChanges.length ? <div className="sm:col-span-2 lg:col-span-5 border-t pt-3"><div className="mb-2 text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Recent changes</div><div className="grid gap-1.5 md:grid-cols-2">{focused.recentChanges.slice(0, 6).map((change) => <div key={change.id} className="rounded-md border bg-background/60 px-3 py-2 text-xs"><div className="font-medium">{String(change.stage ?? "Unknown")}</div><div className="mt-0.5 text-muted-foreground">{String(change.severity ?? "Unknown")} · {new Date(change.createdAt).toLocaleString()}</div></div>)}</div></div> : null}</CardContent></Card> : agentKey ? <EmptyPanel title="Agent evidence not found" detail="The selected agent is not present in the current evidence range." action={<Button size="sm" variant="outline" onClick={() => onNavigate({ view: "agents" })}>Clear focus</Button>} /> : null}
    <Card className="border-border/70 shadow-none"><CardContent className="overflow-x-auto p-0"><table className="w-full min-w-[860px] text-sm"><thead className="bg-muted/20 text-xs text-muted-foreground"><tr><th className="px-4 py-3 text-left">Agent</th><th className="px-3 py-3 text-left">Category</th><th className="px-3 py-3 text-right">Actions</th><th className="px-3 py-3 text-right">Changes</th><th className="px-3 py-3 text-right">Tokens</th><th className="px-3 py-3 text-right">Avg latency</th><th className="px-3 py-3 text-left">Last activity</th></tr></thead><tbody>{rows.map((agent) => <tr key={agent.name} className={`border-t ${agentKeyFor(data, agent) === agentKey ? "bg-primary/[0.04]" : ""}`}><td className="px-4 py-3 font-medium"><button type="button" onClick={() => onNavigate({ view: "agents", agentKey: agentKeyFor(data, agent) })} className="text-primary hover:underline">{agent.name}</button></td><td className="px-3 py-3"><Badge variant="outline">{agent.category}</Badge></td><td className="px-3 py-3 text-right">{agent.actions}</td><td className="px-3 py-3 text-right">{agent.changes}</td><td className="px-3 py-3 text-right">{agent.tokens.toLocaleString()}</td><td className="px-3 py-3 text-right">{agent.averageLatencyMs ? `${agent.averageLatencyMs} ms` : "—"}</td><td className="px-3 py-3 text-xs text-muted-foreground">{agent.lastActivity ? new Date(agent.lastActivity).toLocaleString() : "No activity"}</td></tr>)}</tbody></table></CardContent></Card>
  </div>;
}
