...
function ChangeListPage() {
  ...
  const set = (k: keyof typeof search, v: string) =>
        ...
        ids.includes(x.id)
          ? {
              ...
              approvals: x.approvals.map((a) =>
                a.status === "pending"
                  ? { ...a, status: action === "approve" ? "approved" : "rejected", timestamp: new Date().toISOString(), comment: `Bulk ${action} by ${role}` }
                  : a,
              ),
            }
          : x,
        ...
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              {bulk === "approve" ? <ShieldCheck className="h-5 w-5 text-success" /> : <ShieldAlert className="h-5 w-5 text-destructive" />}
              Confirm bulk {bulk}
            </DialogTitle>
            <DialogDescription>
              You are about to {bulk} {selected.size} change record{selected.size > 1 ? "s" : ""} as{" "}
              <span className="font-semibold text-foreground">{role}</span>. Each record's rollback plan will remain available for its defined window.
            </DialogDescription>
          </DialogHeader>
...
export function RiskChip({ tier, score }: { tier: string; score?: number }) {
  const tone =
    tier === "Critical" ? "bg-destructive text-destructive-foreground border-destructive" :
    tier === "High" ? "bg-destructive/15 text-destructive border-destructive/40" :
    tier === "Medium" ? "bg-warning/20 text-warning-foreground border-warning/50" :
...
export function ModeChip({ mode }: { mode: string }) {
  const tone =
    mode === "Automatic" ? "bg-primary/15 text-primary border-primary/40" :
    mode === "Assisted" ? "bg-info/15 text-info border-info/40" :
