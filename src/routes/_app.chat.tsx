...
function ChatPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-1.75rem)] min-h-0 w-full overflow-hidden bg-white text-foreground dark:bg-background">
      ...
      {historyOpen && (
        <div className="fixed inset-0 z-40 bg-black/30 lg:hidden" role="presentation" onClick={() => setHistoryOpen(false)}>
          <aside className="flex h-full w-[min(86vw,340px)] flex-col border-r bg-background shadow-xl" role="dialog" aria-label="Chat history" onClick={(event) => event.stopPropagation()}>
            <div className="border-b p-4">
              ...
              <Button variant="secondary" className="mt-3 h-9 w-full justify-center font-medium" onClick={() => { setHistoryOpen(false); void startNewChat(); }}>
                <Plus className="mr-2 h-4 w-4" />New chat
              </Button>
            </div>
            <div className="min-h-0 flex-1 overflow-y-auto px-3 py-4">
              ...
              <div className="space-y-1">
                {filteredSessions.map((session) => (
                  <div key={session.id} className="group flex items-center gap-1 rounded-lg">
                    <button type="button" onClick={() => { setHistoryOpen(false); void openSession(session.id); }} className={"min-w-0 flex-1 rounded-lg px-3 py-2.5 text-left text-sm transition hover:bg-muted/60 " + (session.id === sessionId ? "bg-primary/10 text-foreground" : "text-muted-foreground")}>
                      <div className="flex min-w-0 items-center gap-2"><span className="h-2 w-2 shrink-0 rounded-full bg-muted-foreground/30" /><span className="truncate">{session.title || "Untitled chat"}</span></div>
                      {session.id === sessionId && <div className="mt-1 pl-4 text-[11px] text-muted-foreground">Now</div>}
                    </button>
                    <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 text-muted-foreground hover:bg-destructive/10 hover:text-destructive focus-visible:ring-2 focus-visible:ring-ring" onClick={() => setPendingDelete(session)} aria-label={"Delete " + (session.title || "chat")} title="Delete chat"><Trash2 className="h-3.5 w-3.5" /></Button>
                  </div>
                ))}
              </div>
            </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {pendingDelete ? <>This will permanently delete <span className="font-medium text-foreground">{pendingDelete.title || "this conversation"}</span> and its saved messages from your chat history.</> : "This chat will be permanently deleted."}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => { if (pendingDelete) void remove(pendingDelete.id); }}>Delete chat</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
