...
type Result = { answer?: string; analysis?: string; recommendations?: Recommendation[]; sources?: string[]; correlatedSignals?: CorrelatedSignal[]; confidence?: number; actionRequired?: boolean; fetchedAt?: string; department?: string };
...
const suggestions = [
  ...
  "Analyze the current operational risks and recommend actions.",
...
function ChatPage() {
  ...
  const startNewChat = async (requestedDepartment = departmentKey) => {
    try {
      const result = await createSession({ data: { departmentKey: requestedDepartment } });
  ...
  const switchDepartment = async (nextKey: string) => {
    if (nextKey === departmentKey) return;
    await startNewChat(nextKey);
  };
  ...
  return <div className="flex h-[calc(100vh-7rem)] flex-col">
    ...
    <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
      <Card className="hidden min-h-0 overflow-hidden lg:flex lg:flex-col">
        ...
        {departments.length > 1 && <div className="border-b p-3"><div className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Data scope</div><select aria-label="Department data scope" value={departmentKey ?? ""} onChange={(event) => void switchDepartment(event.target.value)} className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm">{departments.map((department) => <option key={department.department_key} value={department.department_key}>{department.display_name}</option>)}</select><p className="mt-1 text-[10px] text-muted-foreground">AI evidence is isolated to this department.</p></div>}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-2">
          {loadingHistory ? <div className="p-4 text-xs text-muted-foreground">Loading history…</div> : sessions.length ? sessions.map((item) => <div key={item.id} className={`group mb-1 flex items-center rounded-lg border ${item.id === sessionId ? "border-primary/40 bg-primary/5" : "border-transparent hover:bg-muted/40"}`}><button className="min-w-0 flex-1 px-3 py-2 text-left" onClick={() => void openSession(item.id)}><div className="truncate text-xs font-medium">{item.title}</div><div className="mt-0.5 flex items-center gap-1 text-[10px] text-muted-foreground"><span>{item.departmentName ?? "Workspace-wide"}</span><span>·</span><span>{new Date(item.updatedAt).toLocaleString()}</span></div></button><Button variant="ghost" size="icon" className="mr-1 h-7 w-7 opacity-60 hover:opacity-100" onClick={() => void remove(item.id)} title="Delete chat"><Trash2 className="h-3.5 w-3.5" /></Button></div>) : <div className="p-4 text-xs text-muted-foreground">No saved chats yet.</div>}
        </div>
        <div className="border-t p-3 text-[10px] text-muted-foreground">No application-level history limit. Chats remain available until you delete them. Each saved session keeps its own department boundary.</div>
      </Card>

      <Card className="flex min-h-0 flex-col overflow-hidden">
        <div ref={scrollRef} className="min-h-0 flex-1 space-y-5 overflow-y-auto p-6">
          {!messages.length && <div className="rounded-xl border border-dashed p-5"><div className="flex items-center gap-2 text-sm font-medium"><Sparkles className="h-4 w-4 text-primary" /> Start a new {activeDepartmentName} analysis</div><p className="mt-1 text-xs text-muted-foreground">This session is isolated to {activeDepartmentName}. Aegis will not use evidence from other departments.</p><div className="mt-4 flex flex-wrap gap-2">{suggestions.map((q) => <button key={q} onClick={() => send(q)} className="rounded-full border bg-background px-3 py-1.5 text-xs text-muted-foreground hover:border-primary/40 hover:text-foreground">{q}</button>)}</div></div>}
          ...
          {mutation.isPending && <div className="flex gap-3"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary text-primary-foreground"><Bot className="h-4 w-4" /></div><div className="rounded-2xl bg-muted/40 px-4 py-3 text-xs text-muted-foreground">Gathering authorized {activeDepartmentName} evidence and analyzing risk…</div></div>}
        </div>
        ...
        <CardContent className="border-t p-3"><form onSubmit={(e) => { e.preventDefault(); send(input); }} className="flex items-end gap-2"><Textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(input); } }} placeholder={`Ask Aegis about ${activeDepartmentName.toLowerCase()}…`} className="min-h-[52px] resize-none" rows={1} disabled={mutation.isPending || !sessionId} /><Button type="submit" size="icon" disabled={!input.trim() || mutation.isPending || !sessionId}><ArrowUp className="h-4 w-4" /></Button></form><div className="mt-2 flex items-center gap-1.5 text-[10px] text-muted-foreground"><ShieldCheck className="h-3 w-3" /> Department-scoped authorized data · no provider mutations from chat</div></CardContent>
      </Card>
    </div>
  </div>;
}
