...
function ProviderForm({ target, form, set }: { target: Provider; form: FormState; set: (key: string, value: string) => void }) {
  ...
  if (id === "sap") return <div className="space-y-3"><div className="rounded-md border bg-muted/30 p-3 text-xs text-muted-foreground">SAP exposes OAuth 2.0 through product/tenant-specific authorization services. Enter the documented endpoints from your SAP BTP or SAP product service binding; CenOps does not guess them.</div><Field label="OAuth Authorization URL" value={form.authorizationUrl} onChange={(v) => set("authorizationUrl", v)} placeholder="https://…/oauth/authorize" /><Field label="OAuth Token URL" value={form.tokenUrl} onChange={(v) => set("tokenUrl", v)} placeholder="https://…/oauth/token" /><Field label="API base URL (optional)" value={form.baseUrl} onChange={(v) => set("baseUrl", v)} /></div>;
  return <div className="space-y-3"><Field label="Client ID / access key" value={form.clientId} onChange={(v) => set("clientId", v)} /><Field label="Client Secret / secret" type="password" value={form.clientSecret} onChange={(v) => set("clientSecret", v)} /><Field label="API base URL (if required)" value={form.baseUrl} onChange={(v) => set("baseUrl", v)} /></div>;
}

function startProvider(target: Provider, form: FormState) {
  const redirectUri = `${window.location.origin}/integrations/oauth/callback`;
  const data = { connectionId: form.integrationId, displayName: form.displayName, environment: form.environment };
  switch (target.id) {
    case "jira": return startJiraOAuth({ data: { ...data, clientId: form.clientId!, clientSecret: form.clientSecret!, redirectUri } });
    case "salesforce": return startSalesforceOAuth({ data: { ...data, clientId: form.clientId!, clientSecret: form.clientSecret, redirectUri, loginUrl: form.baseUrl } });
    ...
    case "gitlab": return startGitLabOAuth({ data: { ...data, instanceUrl: form.baseUrl || "https://gitlab.com", clientId: form.clientId!, clientSecret: form.clientSecret!, redirectUri } });
    ...
    case "snowflake": return startSnowflakeOAuth({ data: { ...data, clientId: form.clientId!, clientSecret: form.clientSecret!, accountUrl: form.accountUrl!, redirectUri } });
    ...
    case "sap": return startSapOAuth({ data: { ...data, clientId: form.clientId!, clientSecret: form.clientSecret!, authorizationUrl: form.authorizationUrl!, tokenUrl: form.tokenUrl!, apiBaseUrl: form.baseUrl, redirectUri } });
    ...
    case "defender": return startDefenderConnection({ data: { ...data, clientId: form.clientId!, clientSecret: form.clientSecret!, customerTenantId: form.customerTenantId } });
    ...
    case "newrelic": return startNewRelicConnection({ data: { ...data, apiKey: form.apiKey!, region: (form.region as "us" | "eu" | "jp") || "us" } });
    ...
    case "workday": return startWorkdayConnection({ data: { ...data, region: (form.region as "us" | "usWcp" | "eu" | "sg" | "uk") || "us", tenantAlias: form.tenantAlias!, clientId: form.clientId!, clientSecret: form.clientSecret!, redirectUri } });
