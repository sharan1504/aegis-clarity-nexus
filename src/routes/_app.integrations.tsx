...
const EMPTY: FormState = { provider: "", displayName: "", environment: "Production", tenant: "", clientId: "", clientSecret: "", accessToken: "", apiToken: "", baseUrl: "", region: "", accessKeyId: "", secretAccessKey: "", sessionToken: "" };
...
function IntegrationsPage() {
  ...
  return <div>
    ...
    <Dialog open={!!target} onOpenChange={(open) => !open && setTarget(null)}><DialogContent className="sm:max-w-lg">{target && <><DialogHeader><DialogTitle>Add {target.name} integration</DialogTitle><DialogDescription>Create a separate instance for an environment such as Production, Development, UAT or Sandbox.</DialogDescription></DialogHeader><div className="space-y-3 py-2"><select aria-label="Platform" value={target.id} onChange={(e) => { const p = catalog?.providers.find((x) => x.id === e.target.value); if (p) setTarget(p); }} className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm">{catalog?.providers.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select><Input placeholder="Integration name (e.g. Genesys Production)" value={form.displayName} onChange={set("displayName")} /><Input placeholder="Environment (Production, Development, UAT…)" value={form.environment} onChange={set("environment")} />{target.id === "aws" ? <><Input placeholder="AWS access key ID" value={form.accessKeyId} onChange={set("accessKeyId")} autoComplete="off" /><Input type="password" placeholder="AWS secret access key" value={form.secretAccessKey} onChange={set("secretAccessKey")} autoComplete="new-password" /><Input type="password" placeholder="AWS session token (optional)" value={form.sessionToken} onChange={set("sessionToken")} autoComplete="off" /><Input placeholder="AWS region (e.g. us-east-1)" value={form.region} onChange={set("region")} /></> : <>{fieldNeeded(target, "tenant") && <Input placeholder="Tenant ID" value={form.tenant} onChange={set("tenant")} />}{fieldNeeded(target, "clientId") && <Input placeholder="Client ID" value={form.clientId} onChange={set("clientId")} />}{fieldNeeded(target, "clientSecret") && <Input type="password" placeholder="Client secret" value={form.clientSecret} onChange={set("clientSecret")} />}{fieldNeeded(target, "baseUrl") && <Input placeholder="Provider base URL" value={form.baseUrl} onChange={set("baseUrl")} />}{fieldNeeded(target, "accessToken") && <Input type="password" placeholder="Provider access token" value={form.accessToken} onChange={set("accessToken")} />}</>}<div className="flex items-center gap-2 rounded-md border p-3 text-xs text-muted-foreground"><ShieldCheck className="h-4 w-4 shrink-0" />Aegis validates the provider before storing the encrypted credentials.</div>{message && <div className="rounded-md border border-destructive/40 bg-destructive/5 p-3 text-sm text-destructive">{message}</div>}</div><DialogFooter><Button variant="outline" onClick={() => setTarget(null)}>Cancel</Button><Button onClick={connect} disabled={busy}>{busy ? <Loader2 className="mr-1.5 h-4 w-4 animate-spin" /> : null}{busy ? "Validating…" : "Connect & verify"}</Button></DialogFooter></>}</DialogContent></Dialog>
  </div>;
}
