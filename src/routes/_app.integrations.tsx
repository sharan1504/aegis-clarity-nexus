  ...
  const connect = async () => {
    ...
    try {
      if (target.id === "genesys") {
        ...
        const result = await startGenesysOAuth({ data: { region: form.region || DEFAULT_GENESYS_REGION, redirectUri, clientId: form.clientId.trim(), clientSecret: form.clientSecret, displayName: form.displayName, environment: form.environment } });
