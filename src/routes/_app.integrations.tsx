...
type FormState = { provider: string; displayName: string; environment: string; tenant: string; clientId: string; clientSecret: string; accessToken: string; apiToken: string; baseUrl: string; region: string; accessKeyId: string; secretAccessKey: string; sessionToken: string };
...
function IntegrationsPage() {
  ...
  const connect = async () => {
    ...
    try {
      if (target.id === "genesys") {
        ...
        const result = await startGenesysOAuth({ data: { region: form.region || DEFAULT_GENESYS_REGION, redirectUri } });
        if (!result.ok) {
  ...
  return <div>
      ...
      <CardContent className="p-0">
        {catalog === null ? <div className="p-8 text-center text-sm text-muted-foreground">Loading integrations…</div> : filtered.length === 0 ? <div className="p-10 text-center text-sm text-muted-foreground">No integration instances match your filters.</div> : <div className="divide-y">
          {filtered.map((connection) => { const provider = catalog.providers.find((p) => p.id === connection.provider); return <button key={connection.id} onClick={() => setSelected(connection)} className="grid w-full grid-cols-1 items-center gap-3 px-4 py-4 text-left transition-colors hover:bg-muted/40 md:grid-cols-[minmax(220px,1.4fr)_140px_130px_150px_110px_24px]">
            <div className="min-w-0"><div className="flex items-center gap-2"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-muted text-sm font-semibold">{provider?.name.slice(0,1) ?? "?"}</div><div className="min-w-0"><div className="truncate font-medium">{connection.display_name || provider?.name || connection.provider}</div><div className="truncate text-xs text-muted-foreground">{connection.external_id || "No external account ID"}</div></div></div></div>
    ...
    <Dialog open={!!selected} onOpenChange={(open) => !open && setSelected(null)}><DialogContent className="sm:max-w-xl"><DialogHeader><DialogTitle>{selected?.display_name || "Integration details"}</DialogTitle><DialogDescription>Configuration and health for this specific integration instance.</DialogDescription></DialogHeader>{selected && catalog && <div className="space-y-4"><div className="grid grid-cols-2 gap-3 rounded-lg border p-4 text-sm"><div><div className="text-xs text-muted-foreground">Platform</div><div className="mt-1 font-medium">{catalog.providers.find((p) => p.id === selected.provider)?.name ?? selected.provider}</div></div><div><div className="text-xs text-muted-foreground">Environment</div><div className="mt-1 font-medium">{selected.environment}</div></div><div><div className="text-xs text-muted-foreground">External account</div><div className="mt-1 break-all font-medium">{selected.external_id || "—"}</div></div><div><div className="text-xs text-muted-foreground">Status</div><div className="mt-1">{statusBadge(selected.status)}</div></div><div><div className="text-xs text-muted-foreground">Last sync</div><div className="mt-1">{relative(selected.last_sync_at)}</div></div><div><div className="text-xs text-muted-foreground">Connected</div><div className="mt-1">{relative(selected.connected_at)}</div></div></div>{selected.last_error && <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">{selected.last_error}</div>}<div className="flex items-center gap-2 rounded-md border p-3 text-xs text-muted-foreground"><ShieldCheck className="h-4 w-4" />Credentials remain server-side and encrypted. Instance data is tenant-scoped.</div></div>}<DialogFooter><Button variant="outline" onClick={() => setSelected(null)}>Close</Button><Button onClick={() => { if (!selected) return; const provider = catalog?.providers.find((p) => p.id === selected.provider); if (provider) { setSelected(null); setTarget(provider); setForm({ ...EMPTY, provider: provider.id, displayName: selected.display_name ?? "", environment: selected.environment ?? "Production" }); } }}>Reconfigure</Button></DialogFooter></DialogContent></Dialog>

    <Dialog open={!!target} onOpenChange={(open) => !open && setTarget(null)}><DialogContent className="sm:max-w-lg">{target && <><DialogHeader><DialogTitle>Add {target.name} integration</DialogTitle><DialogDescription>Create a separate instance for an environment such as Production, Development, UAT or Sandbox.</DialogDescription></DialogHeader><div className="space-y-3 py-2"><select aria-label="Platform" value={target.id} onChange={(e) => { const p = catalog?.providers.find((x) => x.id === e.target.value); if (p) { setTarget(p); setForm((current) => ({ ...current, provider: p.id })); } }} className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm">{catalog?.providers.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select><Input placeholder="Integration name (e.g. Genesys Production)" value={form.displayName} onChange={set("displayName")} /><Input placeholder="Environment (Production, Development, UAT…)" value={form.environment} onChange={set("environment")} />{target.id === "genesys" ? <div className="space-y-1"><label className="text-xs font-medium text-muted-foreground" htmlFor="genesys-region">Genesys Cloud region</label><select id="genesys-region" aria-label="Genesys Cloud region" value={form.region || DEFAULT_GENESYS_REGION} onChange={(e) => setForm((current) => ({ ...current, region: e.target.value }))} className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm">{GENESYS_REGIONS.map((region) => <option key={region.id} value={region.id}>{region.label} — {region.id}</option>)}</select><div className="text-xs text-muted-foreground">Aegis will redirect you to Genesys Cloud to authorize the required read-only scopes. No access token is entered or stored in this form.</div></div> : target.id === "aws" ? <><Input placeholder="AWS access key ID" value={form.accessKeyId} onChange={set("accessKeyId")} autoComplete="off" /><Input type="password" placeholder="AWS secret access key" value={form.secretAccessKey} onChange={set("secretAccessKey")} autoComplete="new-password" /><Input type="password" placeholder="AWS session token (optional)" value={form.sessionToken} onChange={set("sessionToken")} autoComplete="off" /><Input placeholder="AWS region (e.g. us-east-1)" value={form.region} onChange={set("region")} /></> : <>{fieldNeeded(target, "tenant") && <Input placeholder="Tenant ID" value={form.tenant} onChange={set("tenant")} />}{fieldNeeded(target, "clientId") && <Input placeholder="Client ID" value={form.clientId} onChange={set("clientId")} />}{fieldNeeded(target, "clientSecret") && <Input type="password" placeholder="Client secret" value={form.clientSecret} onChange={set("clientSecret")} />}{fieldNeeded(target, "baseUrl") && <Input placeholder="Provider base URL" value={form.baseUrl} onChange={set("baseUrl")} />}{fieldNeeded(target, "accessToken") && <Input type="password" placeholder="Provider access token" value={form.accessToken} onChange={set("accessToken")} autoComplete="off" />}</>}<div className="flex items-center gap-2 rounded-md border p-3 text-xs text-muted-foreground"><ShieldCheck className="h-4 w-4 shrink-0" />Aegis validates the provider before storing the encrypted credentials.</div>{message && <div className="rounded-md border border-destructive/40 bg-destructive/5 p-3 text-sm text-destructive">{message}</div>}</div><DialogFooter><Button variant="outline" onClick={() => setTarget(null)}>Cancel</Button><Button onClick={connect} disabled={busy}>{busy ? <Loader2 className="mr-1.5 h-4 w-4 animate-spin" /> : null}{busy ? "Validating…" : target.id === "genesys" ? "Continue to Genesys" : "Connect & verify"}</Button></DialogFooter></>}</DialogContent></Dialog>
  </div>;
}
