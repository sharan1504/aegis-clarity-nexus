...
import { connectProvider, getProviderCatalog, removeProviderIntegration } from "@/lib/integrations/provider-functions";
...
function fieldNeeded(provider: Provider, field: keyof FormState) {
  if (provider.id === "github") return false;
...
function ProviderPicker({ providers, value, onChange }: { providers: Provider[]; value: string; onChange: (provider: Provider) => void }) {
  ...
  return <div className="relative"><label className="mb-1.5 block text-xs font-medium text-muted-foreground">Platform</label><button type="button" aria-haspopup="listbox" aria-expanded={open} onClick={() => setOpen((current) => !current)} className="flex h-11 w-full items-center gap-3 rounded-md border border-input bg-background px-3 text-left text-sm shadow-sm transition-colors hover:bg-muted/30"><ProviderLogo provider={selected} /><span className="min-w-0 flex-1"><span className="block truncate font-medium">{selected.name}</span><span className="block truncate text-xs text-muted-foreground">{selected.category}</span></span>{selected.availability === "coming_soon" && <Badge variant="secondary" className="hidden sm:inline-flex">Coming soon</Badge>}<ChevronDown className="h-4 w-4 shrink-0 text-muted-foreground" /></button>{open && <div className="absolute z-50 mt-1 w-full overflow-hidden rounded-lg border bg-popover p-2 text-popover-foreground shadow-lg"><div className="relative mb-2"><Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" /><Input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search platforms…" className="pl-9" /></div><div role="listbox" className="max-h-80 overflow-y-auto pr-1">{filtered.length === 0 ? <div className="p-4 text-center text-sm text-muted-foreground">No platforms found.</div> : filtered.map((provider) => <button key={provider.id} type="button" role="option" aria-selected={provider.id === selected.id} onClick={() => { onChange(provider); setOpen(false); setQuery(""); }} className="flex w-full items-center gap-3 rounded-md px-2 py-2.5 text-left transition-colors hover:bg-muted/60"><ProviderLogo provider={provider} /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{provider.name}</span><span className="block truncate text-xs text-muted-foreground">{provider.category} · {provider.description}</span></span>{provider.availability === "available" ? <Badge variant="outline" className="shrink-0 text-[10px]">Available</Badge> : <Badge variant="secondary" className="shrink-0 text-[10px]">Coming soon</Badge>}</button>)}</div></div>}</div>;
}
...
function IntegrationsPage() {
  ...
  const load = async () => {
    try {
      ...
      setMessage("Unable to load integrations.");
    }
  };
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (params.get("github") === "connected") {
        setMessage("GitHub App connected successfully. Repository access is limited to the installation selection.");
        void load();
      }
      if (params.get("github") === "error") setMessage("GitHub connection could not be completed. Please try reconnecting, or contact your Cenops administrator if the problem persists.");
    }
  }, []);
  ...
  const filteredIds = filtered.map((connection) => connection.id);
  ...
  const toggleAllVisible = () => {
    setSelectedIds((current) => {
      const next = new Set(current);
      if (allVisibleSelected) filteredIds.forEach((id) => next.delete(id)); else filteredIds.forEach((id) => next.add(id));
      return next;
    });
  };
  ...
  const requestBulkRemove = () => {
    ...
    if (targets.length) setRemoveTargets(targets);
  };

  const connect = async () => {
    if (!target) return;
    if (target.availability !== "available") { setMessage(`${target.name} is in the Cenops integration catalog and is coming soon. No credentials were requested.`); return; }
    setBusy(true); setMessage(null);
    try {
      if (target.id === "github") {
        const result = await startGitHubAppInstall({ data: { connectionId: form.integrationId, displayName: form.displayName, environment: form.environment } });
        if (!result.ok) {
          setMessage(result.errorMessage.includes("GITHUB_APP_ID") || result.errorMessage.includes("GITHUB_APP_SLUG") || result.errorMessage.includes("GITHUB_APP_PRIVATE_KEY")
            ? "GitHub App setup is temporarily unavailable. Please contact your Cenops administrator. No customer credentials are required for this connection."
            : result.errorMessage);
          return;
        }
      ...
      if (result.ok) { setTarget(null); setForm(EMPTY); await load(); } else setMessage(result.error ?? "Provider connection failed.");
    } catch (error) {
  ...
  const removeConnections = async () => {
    ...
    setBusy(true); setMessage(null);
    try {
      const results = await Promise.allSettled(removeTargets.map(async (connection) => {
        if (connection.provider === "genesys") return deleteGenesysIntegration({ data: { integrationId: connection.id } });
        return removeProviderIntegration({ data: { connectionId: connection.id } });
      }));
      const failed = results.filter((result) => result.status === "rejected" || (result.status === "fulfilled" && !result.value.ok));
      if (failed.length) {
        setMessage(`${failed.length} integration${failed.length === 1 ? "" : "s"} could not be removed. Please retry the failed item${failed.length === 1 ? "" : "s"}.`);
      } else {
        setRemoveTargets([]);
      ...
      setMessage(error instanceof Error ? error.message : "Unable to remove integrations.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return <div>
    ...
    <Card className="mb-4"><CardHeader className="pb-3"><div className="flex flex-col gap-3 lg:flex-row lg:items-center"><div className="relative min-w-0 flex-1"><Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" /><Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search integrations, accounts, environments…" className="pl-9" /></div><div className="flex flex-wrap gap-2"><select aria-label="Provider filter" value={providerFilter} onChange={(e) => setProviderFilter(e.target.value)} className="h-9 rounded-md border border-input bg-background px-3 text-sm"><option value="all">All platforms</option>{catalog?.providers.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select><select aria-label="Status filter" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="h-9 rounded-md border border-input bg-background px-3 text-sm"><option value="all">All statuses</option><option value="connected">Connected</option><option value="failed">Action required</option><option value="disconnected">Disconnected</option></select><select aria-label="Environment filter" value={environmentFilter} onChange={(e) => setEnvironmentFilter(e.target.value)} className="h-9 rounded-md border border-input bg-background px-3 text-sm"><option value="all">All environments</option>{environments.map((e) => <option key={e} value={e}>{e}</option>)}</select><Button onClick={() => { const provider = catalog?.providers.find((item) => item.id === "github") ?? catalog?.providers[0] ?? null; setTarget(provider); setForm({ ...EMPTY, provider: provider?.id ?? "" }); setMessage(null); }}><Plus className="mr-1.5 h-4 w-4" />Add integration</Button></div></div></CardHeader></Card>

    {selectedIds.size > 0 && <div className="mb-3 flex flex-wrap items-center justify-between gap-3 rounded-lg border bg-muted/30 px-4 py-3"><div className="text-sm font-medium">{selectedIds.size} integration{selectedIds.size === 1 ? "" : "s"} selected</div><Button variant="destructive" onClick={requestBulkRemove} disabled={busy}>Remove selected</Button></div>}

    <Card><CardHeader className="pb-2"><div className="flex items-center gap-3"><input type="checkbox" aria-label="Select all visible integrations" checked={allVisibleSelected} ref={(element) => { if (element) element.indeterminate = someVisibleSelected; }} onChange={toggleAllVisible} className="h-4 w-4 cursor-pointer rounded border-input" /><CardTitle className="text-base">Connected integration instances <span className="ml-1 text-sm font-normal text-muted-foreground">{filtered.length}</span></CardTitle></div></CardHeader><CardContent className="p-0">{catalog === null ? <div className="p-8 text-center text-sm text-muted-foreground">Loading integrations…</div> : filtered.length === 0 ? <div className="p-10 text-center text-sm text-muted-foreground">No integration instances match your filters.</div> : <div className="divide-y">{filtered.map((connection) => { const provider = catalog.providers.find((p) => p.id === connection.provider); return <div key={connection.id} role="button" tabIndex={0} onClick={() => setSelected(connection)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setSelected(connection); }} className="grid w-full cursor-pointer grid-cols-1 items-center gap-3 px-4 py-4 text-left transition-colors hover:bg-muted/40 md:grid-cols-[28px_minmax(220px,1.4fr)_140px_130px_150px_110px_24px]"><div onClick={(event) => event.stopPropagation()}><input type="checkbox" aria-label={`Select ${connection.display_name || provider?.name || connection.provider}`} checked={selectedIds.has(connection.id)} onChange={() => toggleSelection(connection.id)} className="h-4 w-4 cursor-pointer rounded border-input" /></div><div className="min-w-0"><div className="flex items-center gap-2">{provider ? <ProviderLogo provider={provider} /> : <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-muted text-sm font-semibold">?</div>}<div className="min-w-0"><div className="truncate font-medium">{connection.display_name || provider?.name || connection.provider}</div><div className="truncate text-xs text-muted-foreground">{connection.external_id || "No external account ID"}</div></div></div></div><div><div className="text-xs text-muted-foreground">Environment</div><Badge variant="secondary" className="mt-1">{connection.environment || "Production"}</Badge></div><div><div className="text-xs text-muted-foreground">Status</div><div className="mt-1">{statusBadge(connection.status)}</div></div><div><div className="text-xs text-muted-foreground">Last sync</div><div className="mt-1 text-sm">{relative(connection.last_sync_at)}</div></div><div className="text-xs text-muted-foreground">Updated {relative(connection.updated_at)}</div><ChevronRight className="h-4 w-4 text-muted-foreground" /></div>; })}</div>}</CardContent></Card>

    <Dialog open={!!selected} onOpenChange={(open) => !open && setSelected(null)}><DialogContent className="sm:max-w-xl"><DialogHeader><DialogTitle>{selected?.display_name || "Integration details"}</DialogTitle><DialogDescription>Configuration and health for this specific integration instance.</DialogDescription></DialogHeader>{selected && catalog && <div className="space-y-4"><div className="grid grid-cols-2 gap-3 rounded-lg border p-4 text-sm"><div><div className="text-xs text-muted-foreground">Platform</div><div className="mt-1 flex items-center gap-2 font-medium">{(() => { const provider = catalog.providers.find((p) => p.id === selected.provider); return provider ? <><ProviderLogo provider={provider} />{provider.name}</> : selected.provider; })()}</div></div><div><div className="text-xs text-muted-foreground">Environment</div><div className="mt-1 font-medium">{selected.environment}</div></div><div><div className="text-xs text-muted-foreground">External account</div><div className="mt-1 break-all font-medium">{selected.external_id || "—"}</div></div><div><div className="text-xs text-muted-foreground">Status</div><div className="mt-1">{statusBadge(selected.status)}</div></div><div><div className="text-xs text-muted-foreground">Last sync</div><div className="mt-1">{relative(selected.last_sync_at)}</div></div><div><div className="text-xs text-muted-foreground">Connected</div><div className="mt-1">{relative(selected.connected_at)}</div></div></div>{selected.last_error && <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">{selected.last_error}</div>}<div className="flex items-center gap-2 rounded-md border p-3 text-xs text-muted-foreground"><ShieldCheck className="h-4 w-4" />Credentials remain server-side and encrypted. Instance data is tenant-scoped.</div></div>}<DialogFooter><Button variant="outline" onClick={() => setSelected(null)}>Close</Button>{selected?.provider === "genesys" && <Button variant="destructive" onClick={() => setRemoveTargets(selected ? [selected] : [])} disabled={busy}>Delete</Button>}{selected && selected.provider !== "genesys" && <Button variant="destructive" onClick={() => setRemoveTargets([selected])} disabled={busy}>Remove integration</Button>}{selected && <Button onClick={() => { const provider = catalog?.providers.find((p) => p.id === selected.provider); if (provider) { setSelected(null); setTarget(provider); setForm({ ...EMPTY, integrationId: selected.id, provider: provider.id, displayName: selected.display_name ?? "", environment: selected.environment ?? "Production" }); } }}>{selected.status === "connected" ? "Reconfigure" : "Reconnect"}</Button>}</DialogFooter></DialogContent></Dialog>

    <AlertDialog open={removeTargets.length > 0} onOpenChange={(open) => !open && setRemoveTargets([])}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{removeTargets.length === 1 ? "Remove integration?" : `Remove ${removeTargets.length} integrations?`}</AlertDialogTitle><AlertDialogDescription>{removeTargets.length === 1 ? `This removes ${removeTargets[0]?.display_name || "this integration"}, its encrypted credentials, and its synced data from Cenops. You can add or reconnect it afterward.` : <>This will remove the following integration instances and their synced data:<div className="mt-3 max-h-40 overflow-y-auto rounded-md border p-2">{removeTargets.map((connection) => <div key={connection.id} className="truncate px-2 py-1 text-sm">{connection.display_name || connection.provider} <span className="text-muted-foreground">· {connection.environment}</span></div>)}</div></>}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={busy}>Cancel</AlertDialogCancel><AlertDialogAction onClick={(event) => { event.preventDefault(); void removeConnections(); }} disabled={busy}>{busy ? <><Loader2 className="mr-1.5 h-4 w-4 animate-spin" />Removing…</> : removeTargets.length === 1 ? "Remove integration" : "Remove integrations"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>

    <Dialog open={!!target} onOpenChange={(open) => !open && setTarget(null)}><DialogContent className="sm:max-w-lg">{target && <><DialogHeader><DialogTitle>{form.integrationId ? `Reconfigure ${target.name} integration` : `Add ${target.name} integration`}</DialogTitle><DialogDescription>Configure this integration instance. Reconfigure keeps the same instance; Add integration creates a new one.</DialogDescription></DialogHeader><div className="space-y-3 py-2"><ProviderPicker providers={catalog?.providers ?? []} value={target.id} onChange={(provider) => { setTarget(provider); setForm((current) => ({ ...current, provider: provider.id })); setMessage(null); }} /><Input placeholder="Integration name (e.g. GitHub Production)" value={form.displayName} onChange={set("displayName")} /><Input placeholder="Environment (Production, Development, UAT…)" value={form.environment} onChange={set("environment")} />{target.id === "github" ? <div className="space-y-3 rounded-md border p-3"><div><div className="text-sm font-medium">Connect with GitHub App</div><div className="mt-1 text-xs text-muted-foreground">Cenops will open GitHub so you can install the Cenops GitHub App and select the repositories it can access. No GitHub access token, client secret, App ID, or private key is entered here.</div></div><div className="rounded-md bg-muted/40 p-3 text-xs text-muted-foreground">Access is limited to the repositories selected during installation. Cenops uses short-lived installation tokens on the server. If the Cenops GitHub App is not available, contact your Cenops administrator.</div></div> : target.id === "genesys" ? <div className="space-y-3 rounded-md border p-3"><div><div className="text-sm font-medium">{form.integrationId ? "Reconfigure Genesys OAuth" : "Fresh Genesys OAuth configuration"}</div><div className="mt-1 text-xs text-muted-foreground">Each Genesys instance uses its own OAuth client. These credentials are stored only on the server and are never returned to the browser.</div></div><Input placeholder="Genesys OAuth Client ID" value={form.clientId} onChange={set("clientId")} autoComplete="off" /><Input type="password" placeholder="Genesys OAuth Client Secret" value={form.clientSecret} onChange={set("clientSecret")} autoComplete="new-password" /><div className="space-y-1"><label className="text-xs font-medium text-muted-foreground" htmlFor="genesys-region">Genesys Cloud region</label><select id="genesys-region" aria-label="Genesys Cloud region" value={form.region || DEFAULT_GENESYS_REGION} onChange={(e) => setForm((current) => ({ ...current, region: e.target.value }))} className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm">{GENESYS_REGIONS.map((region) => <option key={region.id} value={region.id}>{region.label} — {region.id}</option>)}</select></div><div className="text-xs text-muted-foreground">Reconfigure keeps this integration instance ID and OAuth state. A failed authorization will mark this same instance as Action required rather than creating a duplicate.</div></div> : target.id === "aws" ? <><Input placeholder="AWS access key ID" value={form.accessKeyId} onChange={set("accessKeyId")} autoComplete="off" /><Input type="password" placeholder="AWS secret access key" value={form.secretAccessKey} onChange={set("secretAccessKey")} autoComplete="new-password" /><Input type="password" placeholder="AWS session token (optional)" value={form.sessionToken} onChange={set("sessionToken")} autoComplete="off" /><Input placeholder="AWS region (e.g. us-east-1)" value={form.region} onChange={set("region")} /></> : <>{fieldNeeded(target, "tenant") && <Input placeholder="Tenant ID" value={form.tenant} onChange={set("tenant")} />}{fieldNeeded(target, "clientId") && <Input placeholder="Client ID" value={form.clientId} onChange={set("clientId")} />}{fieldNeeded(target, "clientSecret") && <Input type="password" placeholder="Client secret" value={form.clientSecret} onChange={set("clientSecret")} />}{fieldNeeded(target, "baseUrl") && <Input placeholder="Provider base URL" value={form.baseUrl} onChange={set("baseUrl")} />}{fieldNeeded(target, "accessToken") && <Input type="password" placeholder="Provider access token" value={form.accessToken} onChange={set("accessToken")} autoComplete="off" />}</>}<div className="flex items-center gap-2 rounded-md border p-3 text-xs text-muted-foreground"><ShieldCheck className="h-4 w-4 shrink-0" />Cenops validates the provider before storing the encrypted credentials.</div>{target.availability === "coming_soon" && <div className="rounded-md border border-primary/20 bg-primary/5 p-3 text-xs text-muted-foreground"><span className="font-medium text-foreground">Catalog only:</span> {target.name} is visible for discovery and planning, but its production connector is not enabled yet.</div>}{message && <div className="rounded-md border border-destructive/40 bg-destructive/5 p-3 text-sm text-destructive">{message}</div>}</div><DialogFooter><Button variant="outline" onClick={() => setTarget(null)}>Cancel</Button><Button onClick={connect} disabled={busy || target.availability !== "available"}>{busy ? <Loader2 className="mr-1.5 h-4 w-4 animate-spin" /> : null}{busy ? "Connecting…" : target.availability === "coming_soon" ? "Coming soon" : target.id === "genesys" ? "Authorize Genesys" : target.id === "github" ? "Connect GitHub App" : "Connect & verify"}</Button></DialogFooter></>}</DialogContent></Dialog>

    {message && !target && <div className="mt-4 rounded-md border border-destructive/40 bg-destructive/5 p-3 text-sm text-destructive">{message}</div>}
  </div>;
}
