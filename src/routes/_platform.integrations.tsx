...
export const Route = createFileRoute("/_platform/integrations")({
  head: () => pageHead({ path: "/platform/integrations", title: "Integrations — Aegis AI", description: "Manage enterprise integration instances and their health." }),
...
function IntegrationsPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (params.get("github") === "error") setMessage("GitHub connection could not be completed. Please try reconnecting, or contact your Aegis administrator if the problem persists.");
    }
  }, []);
      ...
      if (target.id === "github") {
        ...
        if (!result.ok) {
          setMessage(result.errorMessage.includes("GITHUB_APP_ID") || result.errorMessage.includes("GITHUB_APP_SLUG") || result.errorMessage.includes("GITHUB_APP_PRIVATE_KEY")
            ? "GitHub App setup is temporarily unavailable. Please contact your Aegis administrator. No customer credentials are required for this connection."
            : result.errorMessage);
          return;
        }
