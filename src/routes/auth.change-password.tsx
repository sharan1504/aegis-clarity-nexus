...
export const Route = createFileRoute("/auth/change-password")({ head: () => pageHead({ path: "/auth/change-password", title: "Set your password — Cenops", description: "Create your permanent Cenops password.", noindex: true }), component: ChangePasswordPage });
...
function ChangePasswordPage() {
  ...
  const submit = async (e: React.FormEvent) => { e.preventDefault(); setError(null); if (password.length < 8) { setError("Password must be at least 8 characters."); return; } if (password !== confirm) { setError("Passwords do not match."); return; } setSaving(true); try { const { data: userData, error: updateError } = await supabase.auth.updateUser({ password, data: { force_password_change: false } }); if (updateError) throw updateError; if (!userData.user) throw new Error("Your session could not be updated."); toast.success("Password changed", { description: "Your Cenops account is ready." }); navigate({ to: "/platform" }); } catch (err) { setError(err instanceof Error ? err.message : "Unable to change your password."); } finally { setSaving(false); } };
