...
import { completeConfluenceOAuth } from "@/lib/integrations/oauth-provider.functions";
import { pageHead } from "@/lib/seo";

export const Route = createFileRoute("/integrations/confluence/callback")({
  ssr: false,
  head: () => pageHead({ path: "/integrations/confluence/callback", title: "Connecting Confluence — CenOps", description: "Completing Confluence OAuth authorization." }),
...
function CallbackPage() {
  ...
  const complete = useServerFn(completeConfluenceOAuth);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!search.code || !search.state) throw new Error("Confluence did not return the required OAuth callback parameters.");
