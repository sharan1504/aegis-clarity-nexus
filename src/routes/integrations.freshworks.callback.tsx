...
import { completeFreshworksOAuth } from "@/lib/integrations/oauth-provider.functions";
import { pageHead } from "@/lib/seo";

export const Route = createFileRoute("/integrations/freshworks/callback")({
  ssr: false,
  head: () => pageHead({ path: "/integrations/freshworks/callback", title: "Connecting Freshservice — CenOps", description: "Completing Freshservice OAuth authorization." }),
...
function CallbackPage() {
  ...
  const complete = useServerFn(completeFreshworksOAuth);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!search.code || !search.state) throw new Error("Freshservice did not return the required OAuth callback parameters.");
