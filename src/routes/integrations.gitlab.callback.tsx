...
import { completeGitLabOAuth } from "@/lib/integrations/oauth-provider.functions";
import { pageHead } from "@/lib/seo";

export const Route = createFileRoute("/integrations/gitlab/callback")({
  ssr: false,
  head: () => pageHead({ path: "/integrations/gitlab/callback", title: "Connecting GitLab — CenOps", description: "Completing GitLab OAuth authorization." }),
...
function CallbackPage() {
  ...
  const complete = useServerFn(completeGitLabOAuth);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!search.code || !search.state) throw new Error("GitLab did not return the required OAuth callback parameters.");
