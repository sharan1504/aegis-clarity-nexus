...
import { completeHubSpotOAuth } from "@/lib/integrations/oauth-provider.functions";
import { pageHead } from "@/lib/seo";

export const Route = createFileRoute("/integrations/hubspot/callback")({
  ssr: false,
  head: () => pageHead({ path: "/integrations/hubspot/callback", title: "Connecting HubSpot — CenOps", description: "Completing HubSpot OAuth authorization." }),
...
function CallbackPage() {
  ...
  const complete = useServerFn(completeHubSpotOAuth);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!search.code || !search.state) throw new Error("HubSpot did not return the required OAuth callback parameters.");
