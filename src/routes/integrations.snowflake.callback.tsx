...
export const Route = createFileRoute("/integrations/snowflake/callback")({
  ssr: false,
  head: () => pageHead({ path: "/integrations/snowflake/callback", title: "Connecting Snowflake — CenOps", description: "Completing Snowflake OAuth authorization." }),
...
function CallbackPage() {
  ...
  const complete = useServerFn(completeSnowflakeOAuth);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!search.code || !search.state) throw new Error("Snowflake did not return the required OAuth callback parameters.");
