...
import { completeZohoOAuth } from "@/lib/integrations/oauth-provider.functions";
import { pageHead } from "@/lib/seo";

export const Route = createFileRoute("/integrations/zoho/callback")({
  ssr: false,
  head: () => pageHead({ path: "/integrations/zoho/callback", title: "Connecting Zoho CRM — CenOps", description: "Completing Zoho CRM OAuth authorization." }),
...
function CallbackPage() {
  ...
  const complete = useServerFn(completeZohoOAuth);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!search.code || !search.state) throw new Error("Zoho CRM did not return the required OAuth callback parameters.");
