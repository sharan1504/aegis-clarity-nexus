...
export const Route = createFileRoute("/platform/settings")({ head: () => pageHead({ path: "/platform/settings", title: "Workspace Settings — Cenops", description: "Manage organization, security, appearance, AI safety and workspace preferences." }), component: SettingsPage });
...
function SettingsPage() {
  ...
  return <div><PageHeader title="Settings" description="Persistent workspace configuration. Changes are stored against your tenant." />{loading ? <div className="py-16 text-center text-sm text-muted-foreground">Loading workspace settings…</div> : <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
    ...
    <Card className="xl:col-span-2"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Webhook className="h-4 w-4" /> Outbound webhooks</CardTitle><CardDescription>Receive signed Cenops events asynchronously. Webhook failures never block the originating change or approval.</CardDescription></CardHeader><CardContent className="space-y-5">
      <div className="grid gap-3 md:grid-cols-[1fr_auto]"><Input value={webhookUrl} onChange={(e) => setWebhookUrl(e.target.value)} placeholder="https://your-system.example/cenops/webhook" /><Button onClick={() => void create()} disabled={webhookLoading || !webhookUrl.trim() || webhookEvents.length === 0}>{webhookLoading ? <Loader2 className="mr-1.5 h-4 w-4 animate-spin" /> : <Webhook className="mr-1.5 h-4 w-4" />}Subscribe</Button></div>
