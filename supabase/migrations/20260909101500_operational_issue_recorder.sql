...
BEGIN
  ...
  v_key := p_source::text || ':' || COALESCE(NULLIF(trim(p_related_id), ''), encode(digest(
    lower(trim(p_source::text || '|' || COALESCE(p_title, '') || '|' || COALESCE(p_detail, '') || '|' || COALESCE(p_fingerprint, ''))), 'sha256'), 'hex'));

  SELECT id, status INTO v_id, v_status
  FROM public.operational_issues
  WHERE tenant_id = p_tenant_id AND dedupe_key = v_key
  FOR UPDATE;
  ...
  ELSE
    ...
    SET severity = p_severity,
        ...
        detail = left(trim(p_detail), 10000),
  ...
  IF v_should_notify THEN
    ...
    SELECT p_tenant_id, ur.user_id, 'operational_issue', upper(p_severity::text) || ': ' || left(trim(p_title), 250), left(trim(p_detail), 2000), '/operational-console?issue=' || v_id::text
    FROM public.user_roles ur
    WHERE ur.tenant_id = p_tenant_id AND ur.role = 'admin';
  END IF;
