-- Provider integration removal uses the existing immutable audit trail.
-- provider_connection is an implementation entity, so allow it alongside
-- the public-facing integration entity type.
...
BEGIN
  ...
  END IF;
  IF NEW.entity_type NOT IN ('change_record','change_approval','approval','report','ticket','notification','integration','provider_connection','user','tenant','agent','system','guardrail') THEN
