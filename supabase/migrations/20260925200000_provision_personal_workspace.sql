...
BEGIN
  ...
  SELECT p.tenant_id INTO v_profile_tenant
    FROM public.profiles AS p WHERE p.id = p_user_id FOR UPDATE;
  ...
  IF v_profile_tenant IS NOT NULL THEN
    ...
    SELECT COALESCE(pg_catalog.array_agg(ur.role::text ORDER BY
      CASE ur.role WHEN 'admin'::public.app_role THEN 1 WHEN 'manager'::public.app_role THEN 2 WHEN 'analyst'::public.app_role THEN 3 ELSE 4 END
    ), ARRAY[]::text[]) INTO v_roles
    ...
    RETURN pg_catalog.jsonb_build_object(
      'tenantId', v_tenant.id, 'tenantName', v_tenant.name, 'primaryDomain', v_tenant.primary_domain,
      'roles', v_roles, 'role', v_roles[1], 'environmentMode', v_tenant.environment_mode,
      'created', false, 'via', 'existing'
    );
  END IF;

  SELECT count(DISTINCT ur.tenant_id), min(ur.tenant_id)
    INTO v_membership_count, v_membership_tenant
  ...
  v_slug := pg_catalog.trim(both '-' from v_slug);
