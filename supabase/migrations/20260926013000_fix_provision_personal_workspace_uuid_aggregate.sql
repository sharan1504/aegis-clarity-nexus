-- Fix UUID workspace membership lookup in the first-login provisioning RPC.
-- PostgreSQL does not provide min(uuid); use array_agg because the count is constrained
-- to zero or one tenant before this value is consumed.
...
BEGIN
  ...
  SELECT u.email, NULLIF(u.raw_user_meta_data ->> 'full_name', '') INTO v_email, v_full_name FROM auth.users AS u WHERE u.id = p_user_id;
  ...
  SELECT p.tenant_id INTO v_profile_tenant FROM public.profiles AS p WHERE p.id = p_user_id FOR UPDATE;

  IF v_profile_tenant IS NOT NULL THEN
    SELECT t.* INTO STRICT v_tenant FROM public.tenants AS t WHERE t.id = v_profile_tenant;
    SELECT COALESCE(pg_catalog.array_agg(ur.role::text ORDER BY CASE ur.role WHEN 'admin'::public.app_role THEN 1 WHEN 'manager'::public.app_role THEN 2 WHEN 'analyst'::public.app_role THEN 3 ELSE 4 END), ARRAY[]::text[]) INTO v_roles
    ...
    END IF;
    RETURN pg_catalog.jsonb_build_object('tenantId', v_tenant.id, 'tenantName', v_tenant.name, 'primaryDomain', v_tenant.primary_domain, 'roles', v_roles, 'role', v_roles[1], 'environmentMode', v_tenant.environment_mode, 'created', false, 'via', 'existing');
  END IF;

  SELECT count(DISTINCT ur.tenant_id), (pg_catalog.array_agg(DISTINCT ur.tenant_id))[1]
    INTO v_membership_count, v_membership_tenant FROM public.user_roles AS ur WHERE ur.user_id = p_user_id;
  ...
  IF v_tenant.id IS NULL THEN SELECT t.* INTO STRICT v_tenant FROM public.tenants AS t WHERE t.slug = v_slug; END IF;
  UPDATE public.profiles SET tenant_id = v_tenant.id WHERE id = p_user_id;
  INSERT INTO public.user_roles (user_id, tenant_id, role) VALUES (p_user_id, v_tenant.id, 'admin'::public.app_role) ON CONFLICT (user_id, tenant_id, role) DO NOTHING;
  SELECT COALESCE(pg_catalog.array_agg(ur.role::text ORDER BY CASE ur.role WHEN 'admin'::public.app_role THEN 1 WHEN 'manager'::public.app_role THEN 2 WHEN 'analyst'::public.app_role THEN 3 ELSE 4 END), ARRAY[]::text[]) INTO v_roles
    FROM public.user_roles AS ur WHERE ur.user_id = p_user_id AND ur.tenant_id = v_tenant.id;
  RETURN pg_catalog.jsonb_build_object('tenantId', v_tenant.id, 'tenantName', v_tenant.name, 'primaryDomain', v_tenant.primary_domain, 'roles', v_roles, 'role', v_roles[1], 'environmentMode', v_tenant.environment_mode, 'created', true, 'via', 'provisioned');
END;
$$;
